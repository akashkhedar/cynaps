...
# Build the specific app (Cynaps)
# Output will be in dist/apps/cynaps (lowercase c)
RUN yarn cynaps:build
...
RUN poetry self add poetry-plugin-export \
    ...
    && pip list
# Copy root level scripts if needed, though they seem to be in cynaps/

# We will verify location content copy.

# Copy Frontend Build Artifacts to where Django expects them
# Settings expect: ../../web/dist/apps/Cynaps (relative to cynaps/core)
# So we need to reconstruct /app/web/dist/apps/Cynaps
COPY --from=frontend-builder /app/web/dist/apps/cynaps /app/cynaps/core/static/react-app/
