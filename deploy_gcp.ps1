# deploy_gcp.ps1 - Windows Deployment Script for Cynaps to GCP
...
if ($LASTEXITCODE -ne 0) {
    ...
    gcloud compute instances create $VM_NAME `
        ...
        --tags="http-server,https-server,cynaps-app" `
        --boot-disk-size=40GB `
...
$fwCheck = gcloud compute firewall-rules describe allow-cynaps-ports --format="value(name)" 2>$null
if ($LASTEXITCODE -ne 0) {
    gcloud compute firewall-rules create allow-cynaps-ports `
        --allow="tcp:8080,tcp:9000,tcp:9001" `
        --target-tags=cynaps-app `
        --description="Allow Cynaps Application Ports"
}
...
$items = @("cynaps", "cynaps-sdk", "web", "Dockerfile", "docker-compose.prod.yml", "production.env", "pyproject.toml", "poetry.lock")
