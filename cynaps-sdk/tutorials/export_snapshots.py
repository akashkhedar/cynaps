...
- Convert the export to a specified format (optional if CYNAPS_EXPORT_TYPE is specified)
- Poll for conversion completion
- Download the resulting converted file

Environment variables used:
- CYNAPS_URL: Your Cynaps URL (e.g. https://app.cynaps.xyz)
- CYNAPS_API_KEY: Personal access token (go to your account and generate it)
- CYNAPS_PROJECT_ID: Numeric project ID
- CYNAPS_EXPORT_TYPE: The type of export (CSV, COCO, JSON, etc.) to convert to (optional, default is JSON)

Usage:
```bash
CYNAPS_URL=https://app.cynaps.xyz CYNAPS_API_KEY=your_api_key \
CYNAPS_PROJECT_ID=123456 CYNAPS_EXPORT_TYPE=CSV \
...
from cynaps_sdk.core.api_error import ApiError
...
def export_and_download_snapshot():
    ...
    """
    base_url = os.getenv("CYNAPS_URL")
    api_key = os.getenv("CYNAPS_API_KEY")
    project_id_str = os.getenv("CYNAPS_PROJECT_ID")

    if not base_url or not api_key or not project_id_str:
        logger.error("set CYNAPS_URL, CYNAPS_API_KEY and CYNAPS_PROJECT_ID env vars")
        sys.exit(1)

    try:
        project_id = int(project_id_str)
    except ValueError:
        logger.error("CYNAPS_PROJECT_ID must be an integer")
        sys.exit(1)

    # Initialize v2 client
    client = Cynaps(base_url=base_url, api_key=api_key)
...
def convert_snapshot(export_type: str, export_id: str = None):
    ...
    The conversion API is used to start a conversion on the Cynaps backend, then we poll export status
...
if __name__ == "__main__":
    ...
    export_type = os.getenv("CYNAPS_EXPORT_TYPE", None)
