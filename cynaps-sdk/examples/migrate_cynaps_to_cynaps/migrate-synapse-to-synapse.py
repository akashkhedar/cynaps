"""Migrate LS to Cynaps script.
This migration helps to copy projects from one Cynaps instance to another.
...
**Note:** This code utilizes functions from an older version of the Cynaps SDK (v0.0.34).
The newer versions v1.0 and above still support the functionalities of the old version, but you will need to specify
[`cynaps_sdk._legacy`](../../README.md) in your script.
"""
...
import os
from cynaps_sdk import Client
from cynaps_sdk._legacy.users import User
from cynaps_sdk.data_manager import Filters, Operator, Type, Column
...
class Migration:
    def __init__(self, src_url, src_key, dst_url, dst_key, dest_workspace):
        """Initialize migration that copy projects from one Cynaps instance to another

        :param src_url: source Cynaps instance
        :param src_key: source Cynaps token
        :param dst_url: destination Cynaps instance
        :param dst_key: destination Cynaps token
        :param dest_workspace: destination workspace id
        """
        # Connect to the Cynaps API and check the connection
...
def run():
    ...
    parser = argparse.ArgumentParser(description="Cynaps Project Migration Script")
    parser.add_argument(
        "--src-url",
        dest="src_url",
        type=str,
        default=os.getenv("CYNAPS_URL", ""),
        help="Source Cynaps instance (default from CYNAPS_URL)",
    )
    parser.add_argument(
        "--src-key",
        dest="src_key",
        type=str,
        default=os.getenv("CYNAPS_API_KEY", ""),
        help="Source Cynaps token (default from CYNAPS_API_KEY)",
    )
    parser.add_argument(
        "--dst-url",
        dest="dst_url",
        type=str,
        default=os.getenv("DEST_CYNAPS_URL", ""),
        help="Destination Cynaps instance (default from DEST_CYNAPS_URL)",
    )
    parser.add_argument(
        "--dst-key",
        dest="dst_key",
        type=str,
        default=os.getenv("DEST_CYNAPS_API_KEY", ""),
        help="Destination Cynaps token (default from DEST_CYNAPS_API_KEY)",
    )
    parser.add_argument(
        "--project-ids",
        dest="project_ids",
