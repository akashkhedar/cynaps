...
def poll_for_completed_tasks(ls: Cynaps, project_id: int, freq_sec: int) -> list:
    """
    Every n seconds, yield all tasks that have been updated since the last poll.
    Uses cynaps_sdk >= 1.0.0
    """
    while True:
        print("polling")
...
if __name__ == "__main__":
    url = os.getenv("CYNAPS_URL", "http://localhost:8080")
    api_key = os.getenv("CYNAPS_API_KEY")
    project_id = int(os.getenv("CYNAPS_PROJECT_ID", "1"))
