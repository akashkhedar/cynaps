"""
This is a Python script that exports labeled data from a Cynaps project
and converts it into the YOLO format, including downloading associated images.

## 1. What It Does and How
The script performs the following steps:

1. **Connects to Cynaps:** It uses the Cynaps SDK to connect
to a Cynaps instance using the provided URL and API key.
...
5. **Converts Annotations to YOLO Format:** Utilizes the Cynaps Converter
...
1. **Install Dependencies:** Ensure you have Python installed along with the required packages:
   ```bash
   pip install git+https://github.com/cynaps/cynaps-sdk.git tqdm
   ```

2. **Set Up Credentials:** Provide your CYNAPS_URL, CYNAPS_API_KEY, and PROJECT_ID either
by modifying the script variables or via command-line arguments.

3. **Run the Script:** Execute the script from the terminal:
   ```bash
   python script.py --url 'https://app.Cynaps.com' --api-key 'your-api-key' --project-id 12345
   ```
...
## 3. When to Use It
Use this script when you need to prepare labeled datasets from Cynaps for training YOLO object detection models.
...
import os
from cynaps_sdk.converter import Converter
...
def prepare_export(ls: Cynaps, project_id: int, view_id: int | None = None):
...
def run(url: str, api_key: str, project_id: int):
    logger.info("Connecting to Cynaps.")
    client = Cynaps(base_url=url, api_key=api_key)
    logger.info("Connected to Cynaps successfully.")
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="YOLO Export Script for Cynaps")
    parser.add_argument(
        "--url",
        type=str,
        default=os.getenv("CYNAPS_URL", CYNAPS_URL),
        help="Cynaps URL",
    )
    parser.add_argument(
        "--api-key",
        type=str,
        default=os.getenv("CYNAPS_API_KEY", CYNAPS_API_KEY),
        help="Cynaps API Key",
    )
    parser.add_argument(
        "--project-id",
