...
Install:
    ...
    cd cynaps-sdk
    pip install -e .
    python examples/cynaps_enterprise/assigner.py
...
class BatchAssigner:
    def __init__(self, host, api_key, project_id):
        self.client = Cynaps(base_url=host, api_key=api_key)
...
def start():
    host = os.getenv("CYNAPS_URL", "http://localhost:8080")
    api_key = os.getenv("CYNAPS_API_KEY", "")
    project_id = int(os.getenv("CYNAPS_PROJECT_ID", "182"))

    filter_column = "shortname"
    filter_value = "opossum"
    page_size = 10
    emails = os.getenv(
        "CYNAPS_ASSIGN_EMAILS", "user1@example.com,user2@example.com"
    ).split(",")
