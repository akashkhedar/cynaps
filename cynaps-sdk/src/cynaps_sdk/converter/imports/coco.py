...
from cynaps_sdk.converter.utils import ExpandFullPath
from cynaps_sdk.converter.imports.label_config import generate_label_config

logger = logging.getLogger("root")


def new_task(out_type, root_url, file_name):
    """Create a new Cynaps task structure.
    ...
    Returns:
        dict: Cynaps task structure with image data and empty result array
    """
    return {
...
def create_segmentation(
    ...
    """Convert COCO segmentation annotation to Cynaps polygon format.

    COCO segmentation format: flat array of [x1,y1,x2,y2,...] coordinates
    Cynaps format: array of [x,y] points as percentages
    ...
    Returns:
        dict: Cynaps polygon annotation item
    """
...
def create_keypoints(
    ...
    """Convert COCO keypoints annotation to Cynaps keypoint format.
    ...
    Args:
        ...
        image_height (int): Height of the source image in pixels
        image_width (int): Width of the source image in pixels
        point_width (float): Width/size of keypoints in Cynaps

    Returns:
        list: List of Cynaps keypoint annotation items
    """
    label = categories[int(annotation["category_id"])]
...
def convert_coco_to_ls(
    ...
    """Convert COCO dataset annotations to Cynaps JSON format.

    This function processes a COCO dataset JSON file and converts all annotations
    (bounding boxes, segmentations, keypoints) to Cynaps's format. It supports:
    - Object detection (bounding boxes)
    - Instance segmentation (polygons)
    - Keypoint detection (pose estimation)

    The function automatically generates a Cynaps labeling configuration
    based on the annotation types found in the COCO dataset.

    Args:
        input_file (str): Path to input COCO JSON file
        out_file (str): Path to output Cynaps JSON file
        to_name (str, optional): Object name from Cynaps labeling config. Defaults to "image".
        from_name (str, optional): Control tag name from Cynaps labeling config. Defaults to "label".
    ...
    if len(tasks) > 0:
        ...
        logger.info("Saving Cynaps JSON to %s", out_file)
...
def add_parser(subparsers):
    ...
    coco.add_argument(
        ...
        "--output",
        dest="output",
        help="output file with Cynaps JSON tasks",
        default="output.json",
        action=ExpandFullPath,
    )
    coco.add_argument(
        "--to-name",
        dest="to_name",
        help="object name from Cynaps labeling config",
        default="image",
    )
    coco.add_argument(
        "--from-name",
        dest="from_name",
        help="control tag name from Cynaps labeling config",
        default="label",
    )
    coco.add_argument(
        "--out-type",
