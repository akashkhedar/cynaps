"""This code allows to export Cynaps Export JSON to FUNSD format.
It's only the basic converter, it converts every bbox as a separate word.
Check this github issue for more details:
https://github.com/cynaps/cynaps/issues/2634#issuecomment-1251648670

Usage: funsd.py export.json
This command will export your Cynaps OCR annotations to "./funsd/" directory.
"""
...
def convert_annotation_to_fund(result):
    # collect all Cynaps results and combine labels, text, coordinates into one record
...
def cynaps_to_funsd_converter(
    cynaps_export_path="export.json", funsd_dir="funsd", data_key="ocr"
):
    with open(cynaps_export_path) as f:
        tasks = json.load(f)
...
if __name__ == "__main__":
    ...
    print('This command will export your Cynaps OCR annotations to \"./funsd/\" directory')

    cynaps_to_funsd_converter(sys.argv[1])
