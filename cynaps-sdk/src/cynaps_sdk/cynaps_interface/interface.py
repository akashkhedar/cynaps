...
from cynaps_sdk._legacy.exceptions import (
    LSConfigParseException,
    CynapsXMLSyntaxErrorSentryIgnored,
    CynapsValidationErrorSentryIgnored,
)

from .base import CynapsTag
...
def _fix_choices(config):
    ...
    https://github.com/cynaps/cynaps/issues/1259
    """
...
class LabelInterface:
    ...
    within the Cynaps ecosystem.

    It is designed to be compatible at the data structure level with
    an existing parser widely used within the Cynaps ecosystem.
    ...
    ```python
    from cynaps_sdk.cynaps_interface import LabelInterface
    ...
    # returns a JSON representing a Cynaps region
    region.as_json()
    ...
    def create_instance(cls, *args, **kwargs):
        ...
        Studio JSON format and uploaded to Cynaps
    ...
    def parse(self, config_string: str) -> Tuple[Dict, Dict, Dict, etree._Element]:
        ...
        try:
            ...
            raise CynapsXMLSyntaxErrorSentryIgnored(str(e))
    ...
    def _schema_validation(self, config_string):
        ...
        try:
            ...
            raise CynapsValidationErrorSentryIgnored(str(exc))
            ...
            raise CynapsValidationErrorSentryIgnored(error_message)
        ...
        for toName_ in toNames:
            for toName in toName_.split(","):
                if toName not in names:
                    raise CynapsValidationErrorSentryIgnored(
    ...
    def is_valid(self):
        ...
        try:
            self.validate()
            return True
        except CynapsValidationErrorSentryIgnored:
            return False
    ...
    def validate_config_using_summary(self, summary, strict=False):
        """Validate current config using Cynaps Project Summary"""
    ...
    def validate_annotations_consistency(self, annotations_summary):
        """ """
        # annotations_summary is coming from Cynaps Project Summary, it's
