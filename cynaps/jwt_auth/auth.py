...
class APIKeyAuthScheme(OpenApiAuthenticationExtension):
    ...
    def get_security_definition(self, auto_schema):
        return {
            ...
            'curl https://cynaps-host/api/projects -H "X-API-KEY: syn_xxxxx"'
            '</code></pre>'
            '<br>Or use the Authorization header with ApiKey prefix:'
            '<br><pre><code class="language-bash">'
            'curl https://cynaps-host/api/projects -H "Authorization: ApiKey syn_xxxxx"'
            '</code></pre>',
            'x-fern-header': {
                'name': 'api_key',
                'env': 'CYNAPS_API_KEY',
...
class JWTAuthScheme(OpenApiAuthenticationExtension):
    ...
    def get_security_definition(self, auto_schema):
        return {
            ...
            'You can find your user token on the User Account page in Cynaps. Example: '
            '<br><pre><code class="language-bash">'
            'curl https://cynaps-host/api/projects -H "Authorization: Token [your-token]"'
            '</code></pre>',
            'x-fern-header': {
                'name': 'api_key',
                'env': 'CYNAPS_API_KEY',
