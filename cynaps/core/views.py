...
def sitemap_xml(request):
    ...
    # BASE_DIR is cynaps/core, so static is directly under it
...
INDEXNOW_KEY = '579a080b3263486886e7441628580d70'
...
def main(request, **kwargs):
    ...
    if user.is_authenticated:
        ...
        # Check both the is_annotator flag AND annotator_profile existence
        is_annotator = (hasattr(user, "is_annotator") and user.is_annotator) or hasattr(user, "annotator_profile")
        if not is_annotator:
            if (
...
def version_page(request):
    ...
    # from cynaps.core.utils.common import check_for_the_latest_version
...
def heidi_tips(request):
    ...
    url = "https://raw.githubusercontent.com/Cynaps/cynaps/refs/heads/develop/web/apps/Cynaps/src/components/HeidiTips/liveContent.json"
...
class ContactUsAPI(APIView):
    """
    Handle Contact Us form submissions from the frontend.
    """
    ...
    def post(self, request):
        ...
        try:
            ...
            support_from = "support@cynaps.xyz"
            ...
            # 1. Reply to user
            html_content_user = render_to_string('emails/contact_us_user_reply.html', context)
            text_content_user = strip_tags(html_content_user)
            msg_user = EmailMultiAlternatives(
                "We received your message - Cynaps",
                text_content_user,
                support_from,
                [email],
            ...
            admin_context['name'] = name  # Full name for admin
            html_content_admin = render_to_string('emails/contact_us_admin_notification.html', admin_context)
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Failed to process contact form emails: {e}", exc_info=True)
