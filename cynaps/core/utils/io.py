...
def find_file(file):
    return find_node('cynaps', file, 'file')


def find_dir(directory):
    return find_node('cynaps', directory, 'dir')
...
def validate_upload_url(url, block_local_urls=True):
    """Utility function for defending against SSRF attacks. Raises
        ...
        - CynapsAPIException if the hostname cannot be resolved
    ...
    try:
        ...
        raise CynapsAPIException(f"Can't resolve hostname {domain}")
