...
class CynapsError(Exception):
    pass


class CynapsAPIException(APIException):
    status_code = status.HTTP_500_INTERNAL_SERVER_ERROR
    default_detail = 'Unknown error'


class CynapsDatabaseException(CynapsAPIException):
    default_detail = 'Error executing database query'


class CynapsDatabaseLockedException(CynapsAPIException):
...
class ProjectExistException(CynapsAPIException):
    status_code = status.HTTP_422_UNPROCESSABLE_ENTITY
    default_detail = 'Project with the same title already exists'


class InvalidUploadUrlError(CynapsAPIException):
    default_detail = (
