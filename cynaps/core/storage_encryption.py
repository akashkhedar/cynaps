...
def log_encryption_status():
    ...
    # Database encryption
    encryption_key = getattr(settings, "CYNAPS_ENCRYPTION_KEY", None)
    if encryption_key:
        logger.info("✓ Field-level encryption: CONFIGURED (custom key)")
    else:
        logger.info(
            "⚠ Field-level encryption: Using derived key (set CYNAPS_ENCRYPTION_KEY in production)"
        )
...
CYNAPS_ENCRYPTION_KEY=your-base64-encoded-key-here
