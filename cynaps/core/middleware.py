...
class MultipartStreamMiddleware:
    ...
    # Paths that need early multipart parsing (file uploads)
    MULTIPART_PATHS = ['/import', '/avatar']
    ...
    def __call__(self, request):
        ...
        # Process multipart requests for file upload endpoints
        if 'multipart/form-data' in content_type:
            if any(path in request.path for path in self.MULTIPART_PATHS):
...
class CynapsCspMiddleware(CSPMiddleware):
    """
...
class DownloadPreventionMiddleware:
    ...
    - Requests from Cynaps application (Referer check)
