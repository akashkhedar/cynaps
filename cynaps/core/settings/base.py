...
"""
Django Base settings for Cynaps.
...
try:
    ...
    # Find the root .env file (cynaps-develop/.env)
...
from cynaps.core.utils.params import get_bool_env, get_env, get_env_list, has_env
...
LOGGING = {
    ...
    "formatters": {
        "json": {
            "()": "cynaps.core.utils.formatter.CustomJsonFormatter",
...
from cynaps.core.utils.io import get_data_dir
...
if DOMAIN_FROM_REQUEST:
    ...
    if HOSTNAME and not HOSTNAME.startswith("/"):
        raise ImproperlyConfigured(
            "CYNAPS_HOST must be a subpath if DOMAIN_FROM_REQUEST is True"
        )
...
DJANGO_DB = "default"
DATABASE_NAME_DEFAULT = os.path.join(BASE_DATA_DIR, "cynaps.sqlite3")
...
DATABASES_ALL = {
    ...
    DJANGO_DB_MYSQL: {
        ...
        "NAME": get_env("MYSQL_NAME", "Cynaps"),
...
SPECTACULAR_SETTINGS = {
    "TITLE": "Cynaps API",
    "DESCRIPTION": "Cynaps API for data annotation and labeling",
    "VERSION": "",
...
# Email settings
DEFAULT_FROM_EMAIL = get_env("DEFAULT_FROM_EMAIL", "Cynaps <noreply@cynaps.xyz>")
FROM_EMAIL = DEFAULT_FROM_EMAIL  # Backward compatibility

# SMTP Configuration (only used when EMAIL_BACKEND is smtp)
EMAIL_HOST = get_env("SMTP_HOST", "smtp.zoho.in")
EMAIL_PORT = int(get_env("SMTP_PORT", 465))

# Explicitly handle SSL vs TLS
_SECURE = get_env("SMTP_SECURE", "ssl").lower().strip()
EMAIL_USE_SSL = _SECURE in ["ssl", "true", "yes"] or EMAIL_PORT == 465
EMAIL_USE_TLS = _SECURE in ["tls", "starttls"] or (EMAIL_PORT == 587 and not EMAIL_USE_SSL)

EMAIL_HOST_USER = get_env("NOREPLY_EMAIL_USER", "")  # Default to noreply for global settings
EMAIL_HOST_PASSWORD = get_env("NOREPLY_EMAIL_PASSWORD", "")
EMAIL_TIMEOUT = int(get_env("EMAIL_TIMEOUT", 30))

# Multi-sender configuration
EMAIL_SENDERS = {
    'noreply': {
        'from': get_env("NOREPLY_EMAIL_USER", "noreply@cynaps.xyz"),
        'user': get_env("NOREPLY_EMAIL_USER", ""),
        'password': get_env("NOREPLY_EMAIL_PASSWORD", ""),
    },
    'support': {
        'from': get_env("SUPPORT_EMAIL_USER", "support@cynaps.xyz"),
        'user': get_env("SUPPORT_EMAIL_USER", ""),
        'password': get_env("SUPPORT_EMAIL_PASSWORD", ""),
    },
    'founder': {
        'from': get_env("FOUNDER_EMAIL_USER", "founder@cynaps.xyz"),
        'user': get_env("FOUNDER_EMAIL_USER", ""),
        'password': get_env("FOUNDER_EMAIL_PASSWORD", ""),
    },
}
...
# EDITOR = cynaps-frontend repository
...
REACT_APP_ROOT = os.path.join(BASE_DIR, "../../web/dist/apps/cynaps")
...
if (
    ...
    from cynaps.io_storages.localfiles.functions import (
...
REAL_HOSTNAME = os.getenv(
    "HOSTNAME"
)  # we have to use getenv, because we don't use CYNAPS_ prefix
...
if ENABLE_CSP := get_bool_env("ENABLE_CSP", True):
    ...
    MIDDLEWARE.append("core.middleware.CynapsCspMiddleware")
...
# Enable legacy tokens (useful for running with a pre-existing token via `CYNAPS_USER_TOKEN`)
CYNAPS_ENABLE_LEGACY_API_TOKEN = get_bool_env("CYNAPS_ENABLE_LEGACY_API_TOKEN", False)
...
# Base FSM (Finite State Machine) Configuration for Cynaps
