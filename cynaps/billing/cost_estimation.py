...
1. PROJECT CREATION COST (Project Cost)
   Total Deposit = Security Fee + Storage Fee + Annotation Fee

   Where:
   - Security Fee: 10% of (Storage + Annotation Fee), minimum 500 Credits
...
=====================
- Classification: 2 Credits per task
- Bounding Box: 5 Credits per task
- Polygon/Segmentation: 10 Credits per task
- Keypoint: 8 Credits per task
- Text Annotation: 3 Credits per task
- Audio Transcription: 15 Credits per minute
- Video Annotation: 20 Credits per minute
...
Step 1: Annotation Rate = 5 Credits (bounding box)
Step 2: Complexity Multiplier = 1.5x (6-15 labels)
Step 3: Buffer Multiplier = 1.5x
Step 4: Overlap = 3
Step 5: Annotation Fee = 100 × 5 Credits × 1.5 × 1.5 × 3 = 3,375 Credits
Step 6: Storage Fee = 1 GB × 10 Credits/GB = 10 Credits
Step 7: Security Fee = max(500 Credits, 10% × (10 + 3,375)) = 339 Credits → 500 Credits (min)

Total Deposit = 500 Credits + 10 Credits + 3,375 Credits = 3,885 Credits
...
class CostEstimationService:
    ...
    def calculate_security_fee(cls, storage_fee: Decimal, annotation_fee: Decimal) -> Decimal:
        ...
        Minimum security fee is 500 Credits.
        ...
        Returns:
            Decimal: Security fee amount (minimum 500 Credits)
        """
        ...
        # Apply minimum of 500 Credits
        if security_fee < cls.BASE_DEPOSIT_FEE:
            return cls.BASE_DEPOSIT_FEE
        
        return security_fee
    
    # Annotation type base rates (Credits per task)
    ANNOTATION_RATES = {
    ...
    # Duration-based rates (Credits per minute)
    DURATION_RATES = {
        "audio": Decimal("15"),
        "video": Decimal("20"),
    }
    ...
    def estimate_project_cost(
        ...
        return {
            ...
            "formula_explanation": {
                ...
                "annotation_fee_formula": f"Tasks ({task_count}) × Rate ({annotation_rate} Credits) × Complexity ({complexity_multiplier}) × Buffer (1.5) × Overlap ({overlap})",
                "security_fee_formula": "max(500 Credits, round(10% × (Storage Fee + Annotation Fee)))",
