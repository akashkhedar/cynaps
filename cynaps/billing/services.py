...
class CreditService:
    ...
    def credit_annotator(annotator, organization, total_credits, description=""):
        ...
        logger.info(
            ...
            f"({inr_amount} Credits) for {description}"
        )
...
class ProjectBillingService:
    ...
    BASE_DEPOSIT_FEE = Decimal("500")  # Minimum 500 Credits base fee
    STORAGE_RATE_PER_GB = Decimal("10")  # 10 Credits per GB for deposit calculation
    ANNOTATION_BUFFER_MULTIPLIER = Decimal(
        "1.5"
    )  # 1.5x buffer for estimated annotations
    FREE_STORAGE_GB = Decimal("5")  # 5 GB free storage
    STORAGE_TIER_RATES = {
        "standard": {"min_gb": 5, "max_gb": 50, "rate": Decimal("5")},  # 5 Credits/GB/month
        "large": {"min_gb": 50, "max_gb": 500, "rate": Decimal("3")},  # 3 Credits/GB/month
        "enterprise": {
            "min_gb": 500,
            "max_gb": float("inf"),
            "rate": Decimal("2"),
    ...
    def calculate_security_fee(cls, storage_fee, annotation_fee):
        ...
        Minimum base fee is 500 Credits (fixed).
        If 10% > 500 Credits, round to nearest 50 or 100.
        ...
        Returns:
            Decimal: Base fee amount (minimum 500 Credits)
        """
        from decimal import Decimal, ROUND_HALF_UP
    ...
    def create_project_billing(
        ...
        if billing.available_credits < cls.MIN_BALANCE_FOR_NEW_PROJECT:
            raise InsufficientCreditsError(
                f"Minimum balance of {cls.MIN_BALANCE_FOR_NEW_PROJECT} Credits required to create projects. "
                f"Current balance: {billing.available_credits} Credits"
            )
        ...
        if not billing.has_sufficient_credits(total_deposit):
            raise InsufficientCreditsError(
                ...
                f"Required: {total_deposit} Credits, Available: {billing.available_credits} Credits"
            )
        ...
        logger.info(
            f"Security deposit of {total_deposit} Credits collected for project {project.title} "
    ...
    def charge_import_cost(cls, project, new_task_count, user=None):
        ...
        if not org_billing.has_sufficient_credits(total_cost):
            raise InsufficientCreditsError(
                ...
                f"Required: {total_cost} Credits, Available: {org_billing.available_credits} Credits"
            )
        ...
        logger.info(
            f"Import cost of {total_cost} Credits charged for project {project.title} "
    ...
    def refund_security_deposit(cls, project, reason="Project completed successfully"):
        ...
        logger.info(
            f"Refunded {refundable} Credits security deposit for project {project.title} (ID: {project.id})"
        )
    ...
    def forfeit_security_deposit(cls, project, reason="Project abandoned"):
        ...
        logger.info(
            f"Forfeited {forfeit_amount} Credits security deposit for project {project.title} "
    ...
    def charge_export(
        ...
        if credits_charged > 0:
            if not billing.has_sufficient_credits(credits_charged):
                raise InsufficientCreditsError(
                    f"Insufficient credits for export. Required: {credits_charged} Credits, "
                    f"Available: {billing.available_credits} Credits"
                )
        ...
        if billing.available_credits < remaining_cost_estimate:
            if current_state == ProjectBilling.ProjectState.ACTIVE:
                project_billing.transition_to_state(
                    ...
                    f"Low credits: {billing.available_credits} Credits < estimated remaining {remaining_cost_estimate} Credits",
                )
    ...
    def refund_deleted_tasks(cls, project, task_ids):
        ...
        logger.info(
            f"Refunded {refund_amount} Credits for {slots_refundable} unfilled slots "
        ...
        return {
            ...
            "message": f"Refunded {refund_amount} Credits for {slots_refundable} unfilled annotation slot(s)",
        }
    ...
    def process_project_deletion_refund(cls, project, user=None, reason=None):
        ...
        CreditTransaction.objects.create(
            ...
            description=(
                ...
                f"Base fee: {refund_info['breakdown']['base_fee_refund']:.0f} Credits, "
                f"Buffer: {refund_info['breakdown']['buffer_refund']:.0f} Credits, "
                f"Unannotated tasks: {refund_info['breakdown']['unannotated_tasks_refund']:.0f} Credits"
            ),
            metadata={
        ...
        logger.info(
            f"Project deletion refund: {refund_amount} Credits for project {project.title} (ID: {project.id}). "
