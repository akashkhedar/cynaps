...
def handle_annotation_created(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.debug(
            f"Tracked annotation cost for {instance.id}: {rate} Credits ({annotation_type})"
        )
...
def _process_annotator_payment(
    ...
    try:
        ...
        if not task_assignment.immediate_released:
            ...
            annotator_earnings.inr_equivalent += final_immediate  # 1 credit = 1 Credits
            ...
            logger.info(
                ...
                f"{final_immediate} Credits immediate (task {task.id}, {annotation_type})"
            )
...
def _auto_release_single_annotator_consensus(task):
    ...
    try:
        ...
        logger.info(
            f"Auto-released consensus payment for single annotator {annotator.user.email}: {final_payment} Credits"
        )
