...
class SubscriptionPlan(models.Model):
    ...
    price_inr = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Price in Credits"
    )
    ...
    def __str__(self):
        return f"{self.name} - {self.billing_cycle} ({self.price_inr} Credits)"
...
class CreditPackage(models.Model):
    ...
    rate_per_credit = models.DecimalField(
        max_digits=5, decimal_places=2, help_text="Price per credit in Credits"
    )
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.credits} credits - {self.price_inr} Credits"
...
class Payment(models.Model):
    ...
    def __str__(self):
        return f"{self.organization.title} - {self.amount_inr} Credits ({self.status})"
...
class AnnotatorEarnings(models.Model):
    ...
    def __str__(self):
        return f"{self.annotator.email} - {self.organization.title} ({self.credits_earned} Credits)"
...
class StorageBilling(models.Model):
    ...
    def calculate_charges(self, subscription_plan=None):
        ...
        self.credits_charged = self.net_amount  # 1 currency unit = 1 credit
...
class ProjectBilling(models.Model):
    ...
    def __str__(self):
        return f"{self.project.title} - {self.state} (Project Cost: {self.security_deposit_paid} Credits)"
...
class SecurityDeposit(models.Model):
    ...
    def __str__(self):
        return f"{self.project.title} - {self.total_deposit} Credits ({self.status})"
