...
from core.utils.exceptions import CynapsDatabaseException, ProjectExistException
...
from cynaps_sdk.cynaps_interface.interface import LabelInterface
...
from cynaps.core.utils.common import load_func
...
@method_decorator(
    ...
    To perform most tasks with Cynaps API, you must specify the project ID, sometimes referred to as the `pk`.
    To retrieve a list of your Cynaps projects, update the following command to match your own environment.
...
@method_decorator(
    ...
    decorator=extend_schema(
        ...
        description="""
    Create a project and set up the labeling interface in Cynaps using the API.
    ...
    def get_queryset(self):
        ...
        if user.is_expert:
            try:
                ...
                # Use ExpertProjectAssignment to find projects assigned to this expert
                expert_profile = user.expert_profile
                assigned_project_ids = ExpertProjectAssignment.objects.filter(
                    expert=expert_profile,
                    is_active=True,
                ).values_list('project_id', flat=True)
                
                projects = (
                    Project.objects.filter(id__in=assigned_project_ids)
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_expert:
            try:
                ...
                # Use ExpertProjectAssignment to find projects
