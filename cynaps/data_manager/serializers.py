...
from cynaps.core.utils.common import round_floats
            ...
            if request and hasattr(request, "user") and request.user.is_authenticated:
                ...
                if hasattr(user, "annotator_profile"):
                    ...
                    assignment = TaskAssignment.objects.filter(
                        task=obj, annotator=profile
                    ).first()

                    if assignment and assignment.flagged_for_review:
                        ret["needs_rework"] = True
                        ret["rejection_reason"] = assignment.flag_reason
                    else:
                        ret["needs_rework"] = False
        except Exception as e:
            import logging

            logging.getLogger(__name__).debug(f"Could not add rework info in DataManagerTaskSerializer: {e}")
