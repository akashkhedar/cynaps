...
from users.email_verification import send_contextual_mail
...
def send_organization_invite_email(email, organization, inviter, invite_url, request=None):
    ...
    try:
        ...
        context = {
            ...
            'email': email,
            'site_name': getattr(settings, 'SITE_NAME', 'Cynaps'),
        }
...
def send_project_created_email(project, organization, creator):
    ...
    try:
        ...
        # Send to all members using noreply
        send_contextual_mail(
            subject=subject,
            message=text_message,
