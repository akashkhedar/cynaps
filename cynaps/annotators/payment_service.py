...
class PaymentService:
    ...
    # Base rates per annotation type (in Credits)
    BASE_RATES = {
    ...
    def release_final_payments_on_download(project, task_ids=None, downloaded_by=None):
        ...
        logger.info(
            ...
            f"{processed_count} assignments, {total_released} Credits total"
        )
...
class PayoutService:
    ...
    MINIMUM_PAYOUT = Decimal("1.00")  # TODO: Change back to 100.00 later
    TEST_MINIMUM_PAYOUT = Decimal("1.00")  # Minimum 1 Credit for test payout
    ...
    def validate_payout_request(annotator, amount, is_test=False):
        ...
        if amount < min_amount:
            errors.append(f"Minimum payout amount is {min_amount} Credits")

        # Check available balance (skip for test payouts)
        if not is_test and annotator.available_balance < amount:
            errors.append(
                f"Insufficient balance. Available: {annotator.available_balance} Credits"
            )
    ...
    @staticmethod
    @transaction.atomic
    def create_payout_request(annotator, amount, payout_method="bank_transfer", base_url=None):
        ...
        try:
            ...
            sender_email = "noreply@cynaps.xyz"
            # As requested by the user, send admin emails explicitly to this address
            admin_email = "akashkhedar262@gmail.com"

            # Get the noreply connection so emails are properly authenticated by postfix/zoho
            ...
            # Common context
            user = getattr(annotator, 'user', None)
            name = f"{user.first_name} {user.last_name}".strip() if user else annotator.id
            email_addr = user.email if user else None

            # Generate random transaction ID starting with CYN-PAY-
            import uuid
            transaction_id = f"CYN-PAY-{str(uuid.uuid4())[:8].upper()}"
            
            # Update the payout record with the new transaction ID
            ...
            if payout_method == "upi":
                context["upi_id"] = bank_details.get("upi_id", "")
            else:
                context["bank_name"] = bank_details.get("bank_name", "")
                context["account_number"] = bank_details.get("account_number", "")
                context["ifsc_code"] = bank_details.get("ifsc_code", "")
                context["account_holder"] = bank_details.get("account_holder_name", "")

            connection.open()
            try:
                # 1. Email to Annotator
                if email_addr:
                    annotator_html = render_to_string('emails/payout_request_annotator.html', context)
                    annotator_text = strip_tags(annotator_html)
                    msg_annotator = EmailMultiAlternatives(
                        "Payout Successful - Cynaps",
                        annotator_text,
                        sender_email,
                        [email_addr],
                        connection=connection
                    )
                    msg_annotator.attach_alternative(annotator_html, "text/html")
                    msg_annotator.send(fail_silently=True)

                # 2. Email to Admin
                if admin_email:
                    admin_html = render_to_string('emails/payout_request_admin.html', context)
                    admin_text = strip_tags(admin_html)
                    msg_admin = EmailMultiAlternatives(
                        f"Action Required: New Payout Request (ID: {payout.id})",
                        admin_text,
                        sender_email,
                        [admin_email],
                        connection=connection
            ...
            # We don't want to fail the payout request just because emails failed
            import traceback
            with open(r'f:\cynaps-develop\email_error_log.txt', 'w') as f:
                f.write(traceback.format_exc())
            print(f"CRITICAL EMAIL ERROR: {e}")
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Failed to send payout emails: {e}")
    ...
    @staticmethod
    def create_test_payout(user, amount_credits=1, payout_method="upi"):
        """
        Create a test payout of 1 Credit to verify integration

        Args:
            user: User model instance (annotator or expert)
            amount_credits: Amount in Credits (default 1 Credit)
    ...
    def approve_and_process_payout(payout_request, admin_user):
        ...
        if annotator.available_balance < payout_request.amount:
            return {
                "success": False,
                "error": f"Insufficient balance. Available: {annotator.available_balance} Credits",
            }
    ...
    @staticmethod
    def mark_payout_paid_manually(payout_request, admin_user, transaction_id=None):
        """
        Manually mark a payout as completed without using RazorpayX.
        This is used when the admin makes the transfer manually via their own banking app.
        """
        from .models import EarningsTransaction
        from django.core.mail import EmailMultiAlternatives, get_connection
        from django.template.loader import render_to_string
        ...
        if payout_request.status not in ["pending", "processing"]:
            return {
                "success": False,
                "error": f"Cannot mark payout as paid with status: {payout_request.status}",
            }

        annotator = payout_request.annotator

        # Check if annotator has sufficient balance if it wasn't already deducted
        # (If it's 'processing', balance was already deducted by approve_and_process_payout)
        if payout_request.status == "pending":
            if annotator.available_balance < payout_request.amount:
                return {
                    "success": False,
            ...
            EarningsTransaction.objects.create(
                ...
                amount=-payout_request.amount,
                balance_after=annotator.available_balance,
                description=f"Manual Payout #{payout_request.id} - {payout_request.payout_method}",
            )
        ...
        payout_request.save(update_fields=["status", "transaction_id", "processed_by", "processed_at"])

        # Send confirmation email to Annotator
        try:
            user = annotator.user
            email_addr = user.email if user else None
            ...
            if email_addr:
                ...
                context = {
                    "name": user.first_name if user and user.first_name else "Annotator",
                    "email": email_addr,
                    "amount": float(payout_request.amount),
                    "method": payout_request.payout_method,
                    "payout_id": payout_request.id,
                    "transaction_id": payout_request.transaction_id or transaction_id or "Manual Transfer",
                }

                html_content = render_to_string('emails/payout_completed_annotator.html', context)
                text_content = strip_tags(html_content)

                msg = EmailMultiAlternatives(
                    "Payout Completed - Cynaps",
                    text_content,
                    sender_email,
                    [email_addr],
                    connection=connection
            ...
            logger.error(f"Failed to send payout complete email: {e}", exc_info=True)
