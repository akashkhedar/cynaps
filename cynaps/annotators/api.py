...
from users.email_verification import send_contextual_mail, send_welcome_email
...
class ResendVerificationEmailAPI(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            subject = "Verify Your Email - Cynaps Annotator"
            message = f"""
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            response_data["expert"] = {
                ...
                "expertise_level": "expert",  # All experts are equal
...
class AnnotatorTestSubmitAPI(APIView):
    ...
    def post(self, request):
        ...
        expertise_data = request.data.get("expertise", [])  # New: expertise info for badge creation
        ...
        if client_results and isinstance(client_results, dict):
            ...
            if user:
                ...
                # NEW: Create/update AnnotatorExpertise records for each expertise tested
                for exp in expertise_data:
                    try:
                        ...
                        # Get the category
                        category = ExpertiseCategory.objects.filter(id=category_id).first()
                        if not category:
                            logger.warning(f"Category {category_id} not found, skipping expertise record")
                            continue
                        
                        # Get the specialization if specified
                        specialization = None
                        if specialization_id:
                            specialization = ExpertiseSpecialization.objects.filter(id=specialization_id).first()
                        
                        # Get or create the expertise record
                        expertise_record, exp_created = AnnotatorExpertise.objects.get_or_create(
                            annotator=profile,
                            category=category,
                            ...
                            defaults={
                                ...
                                "test_attempts": 1,
                                "last_test_score": score_percentage,
                                "last_test_at": timezone.now(),
                                "verified_at": timezone.now() if passed else None,
                                "badge_earned": passed,
                                "badge_earned_at": timezone.now() if passed else None,
                            }
                        )
                        
                        if not exp_created:
                            # Update existing record
                            expertise_record.test_attempts += 1
                            expertise_record.last_test_score = score_percentage
                            expertise_record.last_test_at = timezone.now()
                            
                            if passed:
                                expertise_record.status = "verified"
                                expertise_record.verified_at = timezone.now()
                                expertise_record.badge_earned = True
                                expertise_record.badge_earned_at = timezone.now()
                            else:
                                expertise_record.status = "failed"
                            
                            expertise_record.save()
                        
                        # Track earned badges
                        if passed:
                            badge_name = specialization.name if specialization else category.name
                            ...
                            logger.info(f"Badge earned: {badge_name} for user {user.email}")
                    
                    except Exception as e:
                        logger.error(f"Error creating expertise record: {e}")

                # Only update profile status if status is changing to avoid unnecessary db writes
            ...
            response_data = {
                **results,
                ...
                "feedback": feedback,
                "earned_badges": earned_badges,  # New: list of badges earned
            }
            return Response(response_data, status=status.HTTP_200_OK)
...
class AnnotatorAuthCheckAPI(APIView):
    """
    Check if the current user is authenticated as an annotator.
    Used by the React frontend for auth verification.
    """
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {"authenticated": False, "reason": "Not an annotator"},
                status=200,
            )
...
class PayoutRequestAPI(APIView):
    ...
    def post(self, request):
        ...
        base_url = request.build_absolute_uri('/')[:-1] if hasattr(request, 'build_absolute_uri') else None
        result = PayoutService.create_payout_request(profile, amount, payout_method, base_url=base_url)
...
class TestPayoutAPI(APIView):
    """
    Test payout API - Send a test payment of 1 Credits to verify RazorpayX integration.
    ...
    def post(self, request):
        """
        Create a test payout of 1 Credits

        Request body:
        {
            "amount": 1,  # Amount in Credits (default 1 Credits)
        ...
        # Validate amount (max 10 Credits for test payouts)
        if amount > Decimal("10"):
            return Response(
                {"success": False, "error": "Test payout amount cannot exceed 10 Credits"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if amount < Decimal("1"):
            return Response(
                {"success": False, "error": "Minimum test payout is 1 Credits"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class TestPayoutWithTestAccountAPI(APIView):
    ...
    def post(self, request):
        ...
        Request body:
        {
            "amount": 1,  # Amount in Credits (default 1 Credits, max 10 Credits)
        ...
        if amount < Decimal("1") or amount > Decimal("10"):
            return Response(
                {
                    "success": False,
                    "error": "Test payout amount must be between 1 Credits and 10 Credits",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class AdminPayoutManualApproveAPI(APIView):
    """Admin API to manually mark a payout request as completed (bypassing Razorpay)"""
    ...
    def post(self, request, payout_id):
        ...
        result = PayoutService.mark_payout_paid_manually(payout, request.user, transaction_id)
...
class ExpertPayoutRequestAPI(APIView):
    ...
    def post(self, request):
        ...
        if amount < ExpertPaymentConfig.MINIMUM_PAYOUT:
            return Response(
                {"error": f"Minimum payout is {ExpertPaymentConfig.MINIMUM_PAYOUT} Credits"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if amount > expert.available_balance:
            return Response(
                {
                    "error": f"Insufficient balance. Available: {expert.available_balance} Credits"
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class ExpertRequestPayoutAPI(APIView):
    ...
    def post(self, request):
        ...
        if amount > expert.pending_payout:
            return Response(
                {"error": f"Insufficient balance. Available: {expert.pending_payout} Credits"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if amount < expert.minimum_payout:
            return Response(
                {
                    "error": f"Amount must be at least {expert.minimum_payout} Credits (minimum payout threshold)"
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {
                ...
                "message": f"Payout request of {amount} Credits submitted successfully. Processing within 3-5 business days.",
            },
            status=status.HTTP_201_CREATED,
        )
        ...
        with db_transaction.atomic():
            if action == "approve":
                ...
                ExpertEarningsTransaction.objects.create(
                    ...
                    description=f"Payout processed: {payout_request.amount} Credits via {payout_request.payout_method}",
                    metadata={
                        "payout_request_id": payout_request.id,
                ...
                return Response(
                    {
                        "success": True,
                        "message": f"Payout of {payout_request.amount} Credits approved successfully",
                        "transaction_id": transaction_id,
...
class ExpertiseApplyAPI(APIView):
    ...
    def post(self, request):
        ...
        subject = f"Cynaps - {expertise_name} Qualification Test"
...
class UPIRedirectAPI(APIView):
    """
    GET /api/annotators/payouts/<payout_id>/upi-redirect
    
    Renders an HTML page that auto-redirects the user to the actual UPI intent.
    This bypasses email clients like Gmail stripping 'phonepe://' and 'upi://' links.
    """
    ...
    def get(self, request, payout_id):
        ...
        return render(request, 'emails/payout_upi_redirect.html', context)
