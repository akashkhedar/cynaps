...
def create_payout(
    ...
    try:
        ...
        logger.info(f"Created payout: {payout.get('id')} for {amount_inr} Credits")
        return payout
    except razorpay.errors.BadRequestError as e:
...
def process_annotator_payout(payout_request):
    ...
    try:
        ...
        payout = create_payout(
            ...
            narration=f"Cynaps Payout #{payout_request.id}",
            notes={
...
def create_test_payout(email, name, upi_id=None, bank_details=None, amount_inr=1):
    """
    Create a test payout of 1 Credit to verify the integration
    ...
    Args:
        ...
        amount_inr: Amount in Credits (default 1)
