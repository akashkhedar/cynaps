...
class ExpertPaymentConfig:
    ...
    # Base rates per review (in Credits)
    BASE_REVIEW_RATES = {
...
class ExpertService:
    ...
    def assign_expert_role(
        cls, user, assigned_by, expertise_level="expert", expertise_areas=None
    ) -> Dict:
        """
        ...
        Args:
            ...
            expertise_level: Deprecated - all experts are equal now
        ...
        if not created:
            ...
            expert.save(update_fields=["expertise_areas", "status"])
    ...
    def _escalate_to_lead(cls, review_task):
        """Escalate task to another expert (all experts are equal now)"""
        from .models import ExpertProfile, ExpertReviewTask

        # Find any other active expert with lowest workload
        other_expert = (
        ...
        if other_expert:
            # Create new review task for other expert
            ...
            other_expert.current_workload += 1
            other_expert.save(update_fields=["current_workload"])

            logger.info(
                f"Escalated task {review_task.task_id} to expert {other_expert.user.email}"
            )
    ...
    def _process_review_payment(cls, review_task, action) -> Dict:
        ...
        # Level multiplier - all experts are equal now (expertise_level removed)
        level_multiplier = expert.custom_level_multiplier or Decimal("1.0")
        ...
        logger.info(
            f"Processed expert payment for {expert.user.email}: {total_payment} Credits"
        )
    ...
    def accept_consolidated_annotation(
        ...
        logger.info(
            ...
            f"Total released {payment_finalization.get('total_released', 0)} Credits"
        )
