...
class AssignmentEngine:
    ...
    def _performance_based_assignment(project, annotator, required_overlap):
        ...
        with transaction.atomic():
            ...
            from cynaps.tasks.models import Task
            task = (
...
class DynamicAssignmentEngine:
    ...
    def check_and_process_timeouts(cls, project=None):
        ...
        # Track annotators already marked inactive to avoid duplicate processing
        inactive_annotators = set()
        
        count = 0
        for assignment in queryset:
            # Skip if annotator was already marked inactive in this batch
            if assignment.annotator_id in inactive_annotators:
                # Just release this assignment without re-marking annotator
                cls._release_assignment(assignment)
                count += 1
                continue
            
            result = cls.handle_assignment_timeout(assignment)
            if result == 'marked_inactive':
                inactive_annotators.add(assignment.annotator_id)
            count += 1
        
        if count > 0:
            logger.info(f"[Timeout] Processed {count} timed out assignments")
        if inactive_annotators:
            logger.info(f"[Timeout] Marked {len(inactive_annotators)} annotators as inactive")
