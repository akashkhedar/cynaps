...
from cynaps.core.argparser import parse_input_args
from cynaps.core.utils.params import get_env

logger = logging.getLogger(__name__)

CYNAPS_PATH = str(pathlib.Path(__file__).parent.absolute())
DEFAULT_USERNAME = "default_user@localhost"


def _setup_env():
    sys.path.insert(0, CYNAPS_PATH)
...
def main():
    ...
    if input_args.data_dir:
        ...
        os.environ.setdefault("CYNAPS_BASE_DATA_DIR", str(data_dir_path.absolute()))
    ...
    from cynaps.core.utils.common import collect_versions
    ...
    if input_args.command == "version" or input_args.version:
        from cynaps import __version__

        print("\nCynaps version:", __version__, "\n")
    ...
    # init
        ...
        print("")
        print("Cynaps has been successfully initialized.")
        if input_args.command != "start" and input_args.project_name:
            print("Start the server: cynaps start " + input_args.project_name)
            return

    # start with migrations from old projects, '.' project_name means 'cynaps start' without project name
    elif input_args.command == "start" and input_args.project_name != ".":
        from projects.models import Project

        from cynaps.core.old_ls_migration import migrate_existing_project
        ...
        if input_args.project_name and not _project_exists(input_args.project_name):
            ...
            if project_path.exists():
                print("Project directory from previous version of cynaps found")
                ...
                print(
                    Fore.LIGHTYELLOW_EX
                    + "\n*** WARNING! ***\n"
                    + f"Project {input_args.project_name} migrated to Cynaps Database\n"
            ...
            if not migrated:
                print(
                    ...
                    "Did you miss create it first with `cynaps init {project_name}` ?".format(
                        project_name=input_args.project_name
                    )
                )
                return

    # on `start` command, launch browser if --no-browser is not specified and start Cynaps server
    if input_args.command == "start" or input_args.command is None:
        from cynaps.core.utils.common import start_browser
        ...
        if cert_file or key_file:
            logger.error(
                "Cynaps doesn't support SSL web server with cert and key.\n"
