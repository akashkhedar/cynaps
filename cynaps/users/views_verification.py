...
from users.email_verification import resend_verification_email, send_welcome_email
...
def verify_email(request, token):
    ...
    try:
        ...
        if user.is_annotator:
            ...
            # Also update AnnotatorProfile status
            try:
                profile = user.annotator_profile
                profile.email_verified = True
                profile.status = "pending_test"
                profile.save(update_fields=["email_verified", "status"])
                
                # Send welcome email from founder
                send_welcome_email(user, request)
                
                logger.info(
                    f"Email verified for annotator: {user.email}, profile status: {profile.status}"
                )
            except Exception as e:
                logger.warning(f"Could not update annotator profile for {user.email}: {e}")
            ...
            messages.success(
                request,
                "Your email has been verified successfully! Welcome to Cynaps.",
            )
