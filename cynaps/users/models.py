...
class User(UserMixin, AbstractBaseUser, PermissionsMixin, UserLastActivityMixin):
    ...
    @property
    def avatar_url(self):
        """Return avatar URL as an absolute path."""
        if self.avatar:
            # We use root-relative URL from the field
            base_url = self.avatar.url
            
            # Ensure we return an absolute URL for the frontend
            if not base_url.startswith('http'):
                hostname = getattr(settings, 'HOSTNAME', '')
                if hostname:
                    base_url = hostname + base_url
                # No fallback to 8000. Let it be root-relative if HOST is not set,
                # which is better than pointing to the wrong port.
            
            # Minimal cache busting - only if absolutely necessary.
            # Since hash_upload uses UUIDs, filenames are unique per upload.
            return base_url
        return None
