...
for plan_data in PLANS:
    ...
    print(f"  - Price: {plan.price_inr} Credits | Credits: {plan.credits_per_month}/month")
    print(f"  - Storage: {plan.storage_gb} GB free | Extra: {plan.extra_storage_rate_per_gb} Credits/GB")
...
for plan in SubscriptionPlan.objects.filter(is_active=True).order_by('price_inr'):
    ...
    print(f"{plan.name:<25} {plan.price_inr:<10} Credits {plan.credits_per_month:<10} {plan.storage_gb} GB{'':<5} {projects:<10}")
