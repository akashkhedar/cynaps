...
for plan in old_monthly_plans:
    print(f"  - {plan.name}: {plan.price_inr} Credits ({plan.credits_per_month} credits)")
...
for pkg in old_payg:
    print(f"  - {pkg.name}: {pkg.price_inr} Credits ({pkg.credits} credits)")
    pkg.is_active = False
    pkg.save()
...
for plan in active_plans:
    print(f"{plan.name}: {plan.price_inr:,.0f} Credits - {plan.credits_per_month:,} credits/month")
...
for pkg in active_packages:
    print(f"{pkg.name}: {pkg.price_inr:,.0f} Credits - {pkg.credits:,} credits")
