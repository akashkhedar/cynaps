...
export const Labeling = injector(
  observer(({ store, loading }) => {
    ...
    const initLabeling = useCallback(() => {
      if (!SDK.CF) SDK.initSF(sfRef.current);
      SDK.startLabeling();
    }, []);
    ...
    useEffect(() => {
      if ((!SDK.CF && store.dataStore.selected) || isLabelStream) {
        initLabeling();
      }
    }, []);

    useEffect(() => {
      return () => SDK.destroyCF();
    }, []);

    const onResize = useCallback((width) => {
      view.setLabelingTableWidth(width);
      // trigger resize events inside CF
        ...
        <div className={cn("label-view").elem("content").toClassName()}>
          ...
          >
            ...
            <div
              ref={sfRef}
              id="cynaps-dm"
              className={cn("label-view").elem("CF-container").toClassName()}
              key="cynaps"
            />
          </div>
        </div>
