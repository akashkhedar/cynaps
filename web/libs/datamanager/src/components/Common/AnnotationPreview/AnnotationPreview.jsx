...
import { taskToCFFormat } from "../../../sdk/CF-utils";
...
class PreviewGenerator {
  ...
  constructor(labelingConfig) {
    ...
    this.CF = new window.Cynaps(this.root, {
      user: { id: 1 },
      interfaces: [],
  ...
  async processJob() {
    ...
    this.CF.initializeStore(taskToCFFormat(task));
    this.CF.annotationStore.selectAnnotation(annotation.pk ?? annotation.id);
  ...
  async createPreviews(attempts) {
    ...
    try {
      return this.CF.annotationStore.selected.generatePreviews();
    } catch (err) {
      await wait(1000);
