...
export const AppStore = types
  ...
  .views((self) => ({
    ...
    /** @returns {import("../sdk/CF-sdk").CFWrapper} */
    get CF() {
      return self.SDK.CF;
    },
  ...
  .actions((self) => ({
    ...
    setTask: flow(function* ({ taskID, annotationID, pushState }) {
      ...
      // wait for the task to be loaded and CF to be initialized
      yield taskPromise.then(async () => {
        // wait for self.CF to be initialized with currentAnnotation
        let maxWait = 1000;
        while (!self.CF?.currentAnnotation && maxWait > 0) {
          await new Promise((resolve) => setTimeout(resolve, 1));
          maxWait -= 1;
        }

        if (self.CF) {
          const annotation = self.CF?.currentAnnotation;
          const id = annotation?.pk ?? annotation?.id;

          self.CF?.setCFTask(self.taskStore.selected, id);

          if (isFF(FF_REGION_VISIBILITY_FROM_URL)) {
            const { annotation: annIDFromUrl, region: regionIDFromUrl } =
              History.getParams();
            const annotationStore = self.CF?.CF?.annotationStore;
            ...
            if (regionIDFromUrl) {
              const currentAnn = self.CF?.currentAnnotation;
          ...
          console.error("CF not initialized properly");
        }
    ...
    startLabelStream(options = {}) {
      ...
      if (
        isFF(FF_DEV_2887) &&
        self.CF?.CF?.annotationStore?.selected?.commentStore?.hasUnsaved
      ) {
        Modal.confirm({
