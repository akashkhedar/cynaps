...
import { getAnnotationSnapshot } from "../../sdk/CF-utils";
...
export const create = (columns) => {
  const TaskModelBase = DynamicModel("TaskModelBase", columns, {
    ...
    needs_rework: types.optional(types.maybeNull(types.boolean), false),
    rejection_reason: types.optional(types.maybeNull(types.string), null),
      ...
      loadNextTask: flow(function* ({ select = true } = {}) {
        ...
        if (isLabelStream) {
          ...
          console.log(
            ...
            }, user ${getRoot(self).CF.CF.user.id}${
