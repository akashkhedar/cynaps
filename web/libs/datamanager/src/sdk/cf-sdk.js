...
// import { CFHistory } from "./CF-history";
import { annotationToServer, taskToCFFormat } from "./CF-utils";
...
export const SUPPORT_URL = "https://support.Cynaps.com/hc/en-us/requests/new";
export const SUPPORT_URL_REQUEST_ID_PARAM = "tf_37934448633869"; // request_id field ID in ZD

export class CFWrapper {
  /** @type {HTMLElement} */
  root = null;
   ...
   * @param {CFOptions} options
   */
  constructor(dm, element, options) {
    // we need to pass the rest of the options to CF below
    const {
      task,
      preload,
      isLabelStream,
    ...
    console.log(
      "🔍 CFWrapper constructor - canAnnotate:",
      this.canAnnotate,
      "user:",
      user
    );
    ...
    const cfProperties = {
      user: options.user,
      config: this.cfConfig,
      task: taskToCFFormat(this.task),
      ...
      onCynapsLoad: this.onCynapsLoad,
    ...
    this.initCynaps(cfProperties);
  }

  /** @private */
  initCynaps(settings) {
    try {
      const Cynaps = resolveCynaps();

      if (!Cynaps) {
        console.error("Cynaps library not loaded - window.Cynaps is undefined");
        return;
      }

      this.cfInstance = new Cynaps(this.root, settings);

      this.cfInstance.on("presignUrlForProject", this.onPresignUrlForProject);
      ...
      names.forEach((name) => {
        this.datamanager.getEventCallbacks(name).forEach((clb) => {
          this.cfInstance.on(name.replace(/^CF:/, ""), clb);
        });
      });

      if (isFF(FF_DEV_2887)) {
        new CommentsSdk(this.cfInstance, this.datamanager);
      }

      this.datamanager.invoke("cfInit", this, this.cfInstance);
    } catch (err) {
      console.error("Failed to initialize Cynaps", settings);
      console.error(err);
    }
  }
  ...
  async loadTask(taskID, annotationID, fromHistory = false) {
    if (!this.CF) {
      return console.error("Make sure that CF was properly initialized");
    }
    ...
    if (isFF(FF_DEV_2887) && this.CF?.commentStore?.hasUnsaved) {
      Modal.confirm({
  ...
  selectTask(task, annotationID, fromHistory = false) {
    ...
    if (needsAnnotationsMerge) {
      ...
      if (node && isAlive(node)) {
        try {
          ...
          console.warn("[CFWrapper] mergeAnnotations failed:", e);
        }
      }
    }
    ...
    this.setCFTask(task, annotationID, fromHistory);
  }

  setCFTask(task, annotationID, fromHistory, selectPrediction = false) {
    if (!this.CF) return;

    const hasChangedTasks = this.CF?.task?.id !== task?.id && task?.id;

    this.setLoading(true, hasChangedTasks);
    const cfTask = taskToCFFormat(task);
    const isRejectedQueue = isDefined(task.default_selected_annotation);
    const taskList = this.datamanager.store.taskStore.list;
    // annotations are set in CF only and order in DM only, so combine them
    ...
    this.CF.toggleInterface("postpone", task.allow_postpone !== false);
    this.CF.toggleInterface("topbar:task-counter", true);
    this.CF.assignTask(task);
    this.CF.initializeStore(cfTask);
    this.setAnnotation(
      annotationID,
  ...
  setAnnotation(
    ...
    if (!this.CF) {
      console.warn("[CFWrapper] setAnnotation called but CF is not initialized");
      return;
    }
    const id = annotationID ? annotationID.toString() : null;
    const { annotationStore: cs } = this.CF;
    let annotation;
  ...
  saveUserLabels = async () => {
    const body = [];
    const userLabels = this.CF?.userLabels?.controls;
  ...
  async loadUserLabels() {
    // Avoid directly accessing `this.CF.userLabels` synchronously because
    // that property may create observables during Cynaps's init phase
    ...
    const canAccessUserLabels = () => {
      try {
        ...
        if (!("userLabels" in this.CF)) return false;
        return true;
      } catch (err) {
        return false;
      }
    ...
    if (!canAccessUserLabels()) {
      console.warn(
        "[CFWrapper] userLabels not available after waiting; skipping init"
      );
      return;
    }
    ...
    try {
      ...
      console.warn("[CFWrapper] userLabels API failed:", err);
      return;
    }
    ...
    return new Promise((resolve) => {
      const tryInit = async () => {
        try {
          if (!this.CF || !this.CF.userLabels) return;

          // If the userLabels has an `isInitializing` flag, wait until it's false (or timeout)
          try {
            if (typeof this.CF.userLabels.isInitializing !== "undefined") {
              await Promise.race([
                when(() => this.CF.userLabels.isInitializing === false),
          ...
          if (this.CF.userLabels.init) {
            try {
              this.CF.userLabels.init(controls);
            } catch (err) {
              console.warn("[CFWrapper] userLabels.init failed:", err);
            }
          }
        } catch (err) {
          console.warn("[CFWrapper] userLabels init guard failed:", err);
        } finally {
          resolve();
        }
      };
  ...
  onCynapsLoad = async (ls) => {
    console.log("[CFWrapper] onCynapsLoad called with:", ls);
    
    // Guard: if datamanager store was destroyed/removed, skip Cynaps load handling
    if (
      !this.datamanager ||
      !this.datamanager.store ||
      !isAlive(this.datamanager.store)
      ...
      console.warn(
        "[CFWrapper] datamanager.store not alive onCynapsLoad; skipping initialization"
      );
      return;
    }

    this.datamanager.invoke("CynapsLoad", ls);
    this.CF = ls;
    console.log("[CFWrapper] this.CF is now set to:", this.CF);

    if (!this.CF.task) this.setLoading(true);
    ...
    try {
      ...
      // Attempt to set task history; if Cynaps's store is dead this may throw
      try {
        this.CF.setTaskHistory(safeHistory);
      } catch (err) {
        console.warn("[CFWrapper] setTaskHistory failed, skipping:", err);
      }
    } catch (e) {
      // Fallback: set original
      try {
        this.CF.setTaskHistory(_taskHistory);
      } catch (err) {
        console.warn("[CFWrapper] setTaskHistory fallback failed:", err);
      }
    }
  ...
  onStorageInitialized = async (ls) => {
    ...
    if (this.labelStream === false && this.CF) {
      const node = this.getTaskNode();
      if (node && isAlive(node)) {
        const annotationID =
  ...
  saveDraft = async (target = null) => {
    const selected = target || this.CF?.annotationStore?.selected;
  ...
  onUnskipTask = async () => {
    ...
    await this.withinLoadingState(async () => {
      ...
      if (isFF(FF_DEV_3034)) {
        ...
        if (isFF(FF_DEV_2887) && this.CF?.commentStore?.toCache) {
          this.CF.commentStore.toCache(`task.${task.id}`);
        }
  ...
  async submitCurrentAnnotation(
    ...
    if (result && result.id !== undefined) {
      ...
      this.datamanager.invoke(eventName, this.CF, eventData, result);
      ...
      if (
        ...
        this.CF?.commentStore?.persistQueuedComments
      ) {
        await this.CF.commentStore.persistQueuedComments();
      }
    }
  ...
  setLoading(isLoading, shouldReset = false) {
    // Avoid calling into Cynaps which may invoke store mutations when the store is dead
    if (
      !this.datamanager ||
      !this.datamanager.store ||
      !isAlive(this.datamanager.store)
      ...
      console.warn(
        "[CFWrapper] datamanager.store not alive; skipping setLoading"
      );
      return;
    }
    ...
    if (isFF(FF_LSDV_4620_3_ML) && shouldReset) this.CF.clearApp();
    try {
      this.CF.setFlags({ isLoading });
    } catch (err) {
      console.warn(
        "[CFWrapper] CF.setFlags failed or caused store mutation on dead tree:",
        err
      );
    }
    if (isFF(FF_LSDV_4620_3_ML) && shouldReset) this.CF.renderApp();
  }
  ...
  destroy() {
    this.cfInstance?.destroy?.();
    this.cfInstance = null;
  }
  ...
  get taskHistory() {
    return this.CF.taskHistory;
  }

  get currentAnnotation() {
    try {
      return this.CF.annotationStore.selected;
    } catch {
      return null;
    }
  }
  ...
  get annotations() {
    return this.CF.annotationStore.annotations;
  }

  get predictions() {
    return this.CF.annotationStore.predictions;
  }
