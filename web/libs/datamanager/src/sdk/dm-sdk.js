...
import { serializeJsonForUrl, deserializeJsonFromUrl } from "@cynaps/core";
import { isDefined } from "../utils/utils";
import { APIConfig } from "./api-config";
import { createApp } from "./app-create";
import { CFWrapper } from "./CF-sdk";
import { taskToCFFormat } from "./CF-utils";
...
export class DataManager {
  ...
  constructor(config) {
    ...
    this.CynapsOptions = config.Cynaps;
    ...
    this.isAnnotator = config.isAnnotator ?? false;

    // Get canAnnotate from multiple sources
    // Priority: config.canAnnotate > window.APP_SETTINGS.user.can_annotate > config.Cynaps.user.can_annotate > true
    const userCanAnnotate =
      window.APP_SETTINGS?.user?.can_annotate ??
      config.Cynaps?.user?.can_annotate;
    ...
    console.log(
      ...
      "sources:",
      {
        ...
        CynapsUser: config.Cynaps?.user?.can_annotate,
  ...
  on(eventName, callback) {
    if (this.CF && eventName.startsWith("CF:")) {
      const evt = camelCase(eventName.replace(/^CF:/, ""));

      this.CF?.cfInstance?.on(evt, callback);
    }
  ...
  off(eventName, callback) {
    if (this.CF && eventName.startsWith("CF:")) {
      ...
      this.CF?.cfInstance?.off(evt, callback);
    }
  ...
  removeAllListeners() {
    ...
    sfEvents.forEach((evt) => {
      ...
      const eventName = camelCase(evt.replace(/^CF:/, ""));

      callbacks.forEach((clb) => this.CF?.cfInstance?.off(eventName, clb));
    });
  ...
  async invoke(eventName, ...args) {
    if (eventName.startsWith("CF:")) return;
  ...
  initSF(element) {
    ...
    this.CF = new CFWrapper(this, element, {
      ...this.CynapsOptions,
  ...
  /**
   * Initialize CF or use already initialized instance.
   * Render CF interface and load task for labeling.
   * @param {HTMLElement} element Root element CF will be rendered into
  ...
  async startLabeling() {
    ...
    const taskExists = isDefined(this.CF.task) && isDefined(task);
    const taskSelected = this.CF.task?.id === task?.id;
    ...
    if (!isLabelStream && (!taskSelected || isDefined(annotation))) {
      ...
      // this.CF.loadTask(task.id, annotationID);
      this.CF.selectTask(task, annotationID);
    }
  }

  destroyCF() {
    this.invoke("beforeCfDestroy", this, this.CF?.cfInstance);
    this.CF?.destroy();
    this.CF = undefined;
  }

  // Alias for backward compatibility (old name was destroySF)
  destroySF() {
    return this.destroyCF();
  }
  ...
  static taskToCFFormat = taskToCFFormat;
}
