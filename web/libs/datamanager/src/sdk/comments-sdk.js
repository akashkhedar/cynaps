...
export class CommentsSdk {
  ...
  bindEventHandlers() {
    ["comments:create", "comments:update", "comments:delete", "comments:list"].forEach((evt) => this.CF.off(evt));

    this.CF.on("comments:create", this.createComment);
    this.CF.on("comments:update", this.updateComment);
    this.CF.on("comments:delete", this.deleteComment);
    this.CF.on("comments:list", this.listComments);
  }
  ...
  listComments = async (params) => {
    ...
    if (commentUsers.length && this.CF?.store?.enrichUsers) {
      try {
        this.CF.store.enrichUsers(commentUsers);
      } catch (error) {
