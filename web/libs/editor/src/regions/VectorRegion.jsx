 ...
 *
 * Handles conversion between Cynaps's relative coordinates (0-100%) and KonvaVector's image coordinates (pixels).
 *
 * **Coordinate Systems:**
 * - **Cynaps**: Relative coordinates (percentages) for storage and portability
 * - **KonvaVector**: Image coordinates (pixels) for precise editing
 *
 * **Conversion Flow:**
 * ```
 * Cynaps (Relative %) ←→ VectorRegion ←→ KonvaVector (Image px)
 ...
 * - `serialize()`: Exports in Cynaps format with relative coordinates
 */
const Model = types
  .model({
  ...
  .actions((self) => {
    return {
      /**
       * Converts coordinates from Cynaps format (relative %) to KonvaVector format (image pixels)
      ...
      /**
       * Serializes region data in Cynaps format with relative coordinates
       ...
       *
       * @return {VectorRegionResult} The serialized vector region data in Cynaps format
       */
      serialize() {
