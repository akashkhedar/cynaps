...
export const restorePanel = (showComments: boolean): StoredPanelState => {
  ...
  // Force all active panels to the right alignment
  Object.keys(withActiveDefaults).forEach(key => {
    if (!withActiveDefaults[key].detached) {
      withActiveDefaults[key].alignment = Side.right;
    }
  });
