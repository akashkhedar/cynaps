...
const SideTabsPanelsComponent: FC<SidePanelsProps> = ({
  ...
  const padding = useMemo(() => {
    ...
    const allLeftNotVisible = !leftKeys.length || leftKeys.every((key) => !panelData[key].visible);
    const allRightNotVisible = !rightKeys.length || rightKeys.every((key) => !panelData[key].visible);
    ...
    const paddingLeft = panelBreakPoint || panelsHidden || allLeftNotVisible ? 0 : leftCollapsed ? PANEL_HEADER_HEIGHT : visibilityLeft;
    const paddingRight = panelBreakPoint || panelsHidden || allRightNotVisible ? 0 : rightCollapsed ? PANEL_HEADER_HEIGHT : visibilityRight;
