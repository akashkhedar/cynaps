...
import { Cynaps as CynapsReact } from "./Component";
...
export class Cynaps {
  static Component = CynapsReact;

  static instances = new Set<Cynaps>();

  static destroyAll() {
    Cynaps.instances.forEach((inst) => inst.destroy?.());
    Cynaps.instances.clear();
  }
  ...
  // and we can update our other usages of Cynaps to use createRoot, namely tests will likely be affected.
  async createAppV17() {
  ...
  supportLegacyEvents() {
    ...
    console.log(
      "[Cynaps] supportLegacyEvents called, registering callbacks for:",
    ...
    keys.forEach((key) => {
      ...
      if (isDefined(callback)) {
        ...
        console.log(`[Cynaps] Registering event: ${key} -> ${eventName}`);
        this.events.on(eventName, callback);
      }
    });
  }
