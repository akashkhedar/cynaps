...
export default types
  .model("AppStore", {
    ...
    isSubmitting: false,
    /**
     * Flag for disable task in Cynaps
     */
  ...
  .actions((self) => {
    ...
    function afterCreate() {
      ...
      // Event name must be lowercase "cynapsLoad" to match what supportLegacyEvents registers
      // (it uses camelCase which lowercases the first letter)
      console.log("[AppStore] afterCreate invoking cynapsLoad event");
      const events = getEnv(self).events;
      console.log("[AppStore] events object:", events, "hasEvent:", events?.hasEvent?.("cynapsLoad"));
      events.invoke("cynapsLoad", self);
    }
    ...
    function initializeStore({ annotations = [], completions = [], predictions = [], annotationHistory }) {
      ...
      if (self.simpleInit) {
        ...
        console.log("CF: deserialization is finished");
