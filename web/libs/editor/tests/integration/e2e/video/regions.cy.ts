import { Labels, Cynaps, Sidebar, VideoView } from "@cynaps/frontend-test/helpers/CF/index";
...
describe("Video segmentation", suiteConfig, () => {
  ...
  it("Should be invisible out of the lifespan (rectangle)", () => {
    Cynaps.params().config(simpleVideoConfig).data(simpleVideoData).withResult(simpleVideoResult).init();
    Cynaps.waitForObjectsReady();
    // Wait for video and regions to be fully loaded
