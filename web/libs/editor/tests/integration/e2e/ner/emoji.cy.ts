import { Labels, Cynaps } from "@cynaps/frontend-test/helpers/CF";
...
describe("NER - Emoji - Text", () => {
  ...
  it("Should calculate offsets by code points in text", () => {
    Cynaps.params().config(simpleTextConfig).data(simpleTextData).withResult([]).init();
    ...
    Cynaps.serialize().then((results) => {
      ...
      Cynaps.params().config(simpleTextConfig).data(simpleTextData).withResult(results).init();
      Cynaps.waitForObjectsReady();
      RichText.hasRegionWithText("test");
  ...
  it("Should calculate offsets by code points in multiline text", () => {
    Cynaps.params().config(simpleTextConfig).data(multilineTextData).withResult([]).init();
    Cynaps.waitForObjectsReady();
    Labels.select("region");
    ...
    Cynaps.serialize().then((results) => {
      ...
      Cynaps.params().config(simpleTextConfig).data(multilineTextData).withResult(results).init();
      Cynaps.waitForObjectsReady();
      RichText.hasRegionWithText("Warning:");
      RichText.hasRegionWithText("🐱 This is a test");
  ...
  it("Should calculate global offsets by code points and relative offsets by string length in hypertext", () => {
    Cynaps.params().config(simpleHyperTextConfig).data(simpleHyperTextData).withResult([]).init();
    Cynaps.waitForObjectsReady();
    Labels.select("region");
    RichText.selectText("test");
    RichText.hasRegionWithText("test");
    Cynaps.serialize().then((results) => {
      ...
      Cynaps.params().config(simpleHyperTextConfig).data(simpleHyperTextData).withResult(results).init();
      Cynaps.waitForObjectsReady();
      RichText.hasRegionWithText("test");
  ...
  it("Heuristic edge case", () => {
    Cynaps.params().config(simpleHyperTextConfig).data({ text: "<p>🐱\nmeans cat</p>" }).withResult([]).init();
    Cynaps.waitForObjectsReady();
    Labels.select("region");
