import { AudioView, Cynaps, Paragraphs } from "@cynaps/frontend-test/helpers/CF";
import { Network } from "@cynaps/frontend-test/helpers/utils";
...
describe("Sync buffering playback", () => {
  ...
  it.skip("should go though all paragraphs during playback with buffering", () => {
    let attempts = 3;
    const testScenario = () => {
      Cynaps.params().config(videoAudioParagraphsConfig).data(fullOpossumSnowData).withResult([]).init();
