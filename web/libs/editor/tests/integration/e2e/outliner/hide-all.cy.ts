import { Cynaps, Sidebar, Tooltip } from "@cynaps/frontend-test/helpers/CF/index";
...
describe("Outliner - Hide all regions", () => {
  it("should exist", () => {
    Cynaps.params().config(simpleRegionsConfig).data(simpleRegionsData).withResult(simpleRegionsResult).init();
  ...
  it("should be disabled without existed regions", () => {
    Cynaps.params().config(simpleRegionsConfig).data(simpleRegionsData).withResult([]).init();
  ...
  it("should hide all regions except the target region by ID from param", () => {
    ...
    cy.window().then((window: any | unknown) => {
      window.Htx.annotationStore.annotations[0].regionStore.setRegionVisible(window.CF_CONFIG.region);
    });
  ...
  it("should hide all regions except the target region by ID within the targeted annotation tab specified by param", () => {
    ...
    cy.window().then((window: any | unknown) => {
      const annIdFromParam = window.CF_CONFIG.annotation;
      const annotations = window.Htx.annotationStore.annotations;
      const cfAnnotation = annotations.find((ann: any) => ann.pk === annIdFromParam || ann.id === annIdFromParam);
      const annID = cfAnnotation.pk ?? cfAnnotation.id;

      expect(annID).to.equal("10");

      // Move to the annotation tab specified by param
      cy.get('[class="CF-annotations-list__toggle"]').click();
      cy.get('[class="CF-annotations-list__entity-id"]').contains("10").click();

      annotations[1].regionStore.setRegionVisible(window.CF_CONFIG.region);
    });
  ...
  it("should not hide regions in the non-targeted annotaion tab", () => {
    ...
    cy.window().then((window: any | unknown) => {
      window.Htx.annotationStore.annotations[1].regionStore.setRegionVisible(window.CF_CONFIG.region);
    });

    // Validate the annotation tab
    cy.get('[class="CF-annotations-list__entity-id"]').should("contain.text", "20");
  ...
  it("should select the target region by ID from param", () => {
    ...
    cy.window().then((window: any | unknown) => {
      window.Htx.annotationStore.annotations[0].regionStore.selectRegionByID(window.CF_CONFIG.region);
    });
