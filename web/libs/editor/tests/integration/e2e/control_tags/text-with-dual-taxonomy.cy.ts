import { Cynaps, Sidebar, useTaxonomy } from "@cynaps/frontend-test/helpers/CF";
...
describe("Control Tags - Text with Dual Taxonomy", () => {
  it("should select options from two taxonomies and create text region", () => {
    ...
    cy.log("Initialize Cynaps with text and dual taxonomy configuration");
    Cynaps.params().config(textWithDualTaxonomyConfig).data(simpleTextData).withResult([]).init();
    ...
    Cynaps.serialize().then((results: any[]) => {
