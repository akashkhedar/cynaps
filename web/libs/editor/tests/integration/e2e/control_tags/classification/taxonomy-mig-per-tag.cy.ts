import { Cynaps, ImageView, Taxonomy, ToolBar, Modals } from "@cynaps/frontend-test/helpers/CF";
import {
...
describe("Classification - MIG perTag - Taxonomy", () => {
  it("should not have item_index in result", () => {
    Cynaps.params().config(perTagMIGTaxonomyConfig).data(simpleMIGData).withResult([]).init();
  ...
  it("should load perTag result correctly", () => {
    Cynaps.params().config(perTagMIGTaxonomyConfig).data(simpleMIGData).withResult(perTagTaxonomyResult).init();
  ...
  it("should require result", () => {
    Cynaps.params().config(requiredPerTagMIGTaxonomyConfig).data(simpleMIGData).withResult([]).init();
