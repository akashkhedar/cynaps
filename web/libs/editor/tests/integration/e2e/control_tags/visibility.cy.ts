import { useChoices, Cynaps, Textarea, ImageView, Sidebar, Choices } from "@cynaps/frontend-test/helpers/CF";
import {
...
const checkVisibility = (visibleIndexes: number[], totalIndexes: number) => {
  Array.from({ length: totalIndexes }).forEach((_, i) => {
    cy.get(`.CF-choices:eq(${i})`).should(visibleIndexes.includes(i) ? "be.visible" : "not.be.visible");
  });
};

describe("Visibility", () => {
  it("Ensure correct visibility of conditionally selected choices", () => {
    Cynaps.params().config(simpleVisibleWhenVisibilityConfig).data(visibilityTextData).withResult([]).init();
  ...
  it("Ensure correct visibility of conditionally selected choices with parent visibility restrictions", () => {
    Cynaps.params().config(multipleChainedViewsVisibilityConfig).data(visibilityTextData).withResult([]).init();
  ...
  it("Ensure correct visibility of conditionally selected and submitted textarea", () => {
    Cynaps.params().config(textareaVisibilityConfig).data(visibilityTextData).withResult([]).init();
