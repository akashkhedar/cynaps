...
Scenario("Check Rect region for Image", async ({ I, Cynaps, AtImageView, AtOutliner, AtPanels }) => {
  const params = {
    config,
    data: { image },
...
Scenario("Image with perRegion tags", async ({ I, Cynaps, AtOutliner }) => {
  let result;
  const params = {
...
Data(outOfBoundsFFs).Scenario(
  ...
  async ({ I, AtLabels, AtOutliner, AtImageView, Cynaps, AtPanels, current }) => {
