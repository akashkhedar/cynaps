...
Scenario(
  ...
  async ({ I, Cynaps, AtOutliner, AtParagraphs, AtLabels }) => {
    await retryScenario(async () => {
      const params = { data: DATA, config: CONFIG };
      I.amOnPage("/");
      Cynaps.setFeatureFlags(FEATURE_FLAGS);
      Cynaps.init(params);
      AtOutliner.seeRegions(0);

      // Debug: Wait for any label to appear and print all label texts
      I.waitForElement(".CF-label", 10);
      const labelTexts = await I.grabTextFromAll(".CF-label");
      I.say(`Visible labels: ${JSON.stringify(labelTexts)}`);

      // Wait for the specific label to appear
      I.waitForElement(locate(".CF-label").withText("General: Positive1"), 10); // Increased timeout
...
Scenario("Select All button is disabled when no label is selected", async ({ I, Cynaps, AtOutliner }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Cynaps.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey for Select All creates region", async ({ I, Cynaps, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Cynaps.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey: Next Phrase moves focus to next phrase", async ({ I, Cynaps, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Cynaps.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey: Previous Phrase moves focus to previous phrase", async ({ I, Cynaps, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Cynaps.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey: Select All and Annotate creates region", async ({ I, Cynaps, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Cynaps.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey: Next Region in Phrase navigates to next region", async ({ I, Cynaps, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Cynaps.setFeatureFlags(FEATURE_FLAGS);
...
Scenario(
  ...
  async ({ I, Cynaps, AtOutliner, AtLabels }) => {
    await retryScenario(async () => {
      const params = { data: DATA, config: CONFIG };
      I.amOnPage("/");
      Cynaps.setFeatureFlags(FEATURE_FLAGS);
...
Scenario("Hotkey: Next/Previous Region loops at ends", async ({ I, Cynaps, AtOutliner, AtLabels }) => {
  await retryScenario(async () => {
    const params = { data: DATA, config: CONFIG };
    I.amOnPage("/");
    Cynaps.setFeatureFlags(FEATURE_FLAGS);
    ...
    I.click(locate(".CF-outliner-item").at(3));
    ...
    const selectedIdxAfterNext = await I.executeScript(() => {
      const items = Array.from(document.querySelectorAll(".CF-outliner-item"));
    ...
    I.click(locate(".CF-outliner-item").at(1));
