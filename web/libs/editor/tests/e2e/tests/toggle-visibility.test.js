...
const ONE_VISIBLE_SELECTOR = ".CF-tree__node:not(.CF-tree__node_type_label):not(.CF-tree__node_hidden)";
const ONE_HIDDEN_SELECTOR = ".CF-tree__node:not(.CF-tree__node_type_label).CF-tree__node_hidden";
const ONE_VISIBILITY_TOGGLE_BUTTON = ".CF-outliner-item__control_type_visibility button";
...
Scenario("Checking mass toggling of visibility", async ({ I, Cynaps, AtOutliner }) => {
  ...
  await I.amOnPage("/");
  Cynaps.init({ annotations, config, data });
...
Scenario("Hiding bulk visibility toggle", ({ I, Cynaps, AtImageView, AtLabels, AtOutliner }) => {
  I.amOnPage("/");
  Cynaps.init({ config, data });
  Cynaps.waitForObjectsReady();
...
Scenario("Checking regions grouped by label", async ({ I, Cynaps }) => {
  const checkVisible = async (num) => {
    switch (num) {
      case 0:
        I.seeElement(ALL_HIDDEN_SELECTOR);
  ...
  I.waitForElement(".CF-view-controls", 5);

  // Switch to label grouping using UI interaction
  I.click('[data-testid="grouping-manual"]');
  I.waitForElement(".CF-menu", 2);

  I.waitForElement(".CF-dropdown.CF-visible");
  // Click on the "Group by Label" option in the dropdown
  I.click('//div[contains(@class, "CF-view-controls__label") and contains(text(), "Group by Label")]');
