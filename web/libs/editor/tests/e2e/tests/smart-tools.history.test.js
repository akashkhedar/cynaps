...
function getRectangleSuggestions(reg, group) {
  ...
  const annotation = window.Cynaps.store.annotationStore.selected;
  ...
  window.Cynaps.store.loadSuggestions(new Promise((resolve) => resolve(suggestions)), (x) => x);
}

Scenario("Undo regions auto-annotated from predictions", async ({ I, Cynaps, AtImageView, AtOutliner }) => {
  I.amOnPage("/");
  Cynaps.init({
...
Scenario(
  ...
  async ({ I, Cynaps, AtImageView, AtOutliner, AtPanels }) => {
