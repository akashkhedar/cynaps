...
Data(imageExamples).Scenario("Classification Readonly Annotations", async ({ I, current, Cynaps }) => {
  ...
  I.seeElement(".CF-number input:disabled");
  ...
  I.dontSee('.CF-text-area [aria-label="Edit Region"]');

  I.say("Check region is not deletable");
  I.dontSee('.CF-text-area [aria-label="Delete Region"]');

  /**
   * Checking the Choices
   */
  I.say("Checking the Choices");
  I.seeElement(".CF-choices input:disabled");

  I.say("Try selecting anyways");
  I.checkOption("Choice 1", ".CF-choices");
