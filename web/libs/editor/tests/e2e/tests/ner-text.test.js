...
Scenario("NERText", async ({ I, Cynaps, AtOutliner, AtTopbar }) => {
  const params = {
...
Scenario("NER Text with text field missing", async ({ I, Cynaps }) => {
  const params = {
...
Scenario("NER Text from url", async ({ I, Cynaps }) => {
  const params = {
...
Scenario("NER Text from url with text saved", async ({ I, Cynaps }) => {
  const params = {
...
Scenario("NER Text with SECURE MODE and wrong valueType", async ({ I, Cynaps }) => {
  const params = {
    annotations: [{ id: "TestCmpl", result: results }],
    config: configSimple,
    data: { url },
...
Scenario("NER Text with SECURE MODE", async ({ I, Cynaps }) => {
  const params = {
    annotations: [{ id: "TestCmpl", result: resultsFromUrl }],
    config: configUrl,
    data: { url },
  ...
  I.waitForElement(".CF-richtext__line", 60);
...
Scenario("NER Text regions in Outliner", async ({ I, Cynaps }) => {
  const params = {
    annotations: [{ id: "TestCmpl", result: resultsFromUrl }],
    config: configUrl,
    data: { url },
  ...
  I.seeElement(locate(".CF-outliner-item").withText(resultsFromUrl[0].value.text));
  I.seeElement(locate(".CF-outliner-item").withText(resultsFromUrl[1].value.text));
});
