...
Scenario("Make a duplicate of annotation with preselected choices", async ({ I, Cynaps, AtTopbar }) => {
  const params = {
    config: `
<View>
  ...
  const duplicateResult = await Cynaps.serialize();
  ...
  const annotationWithPresetValues = await Cynaps.serialize();
...
Scenario("Make a duplicate of empty annotation with preselected choices", async ({ I, Cynaps, AtTopbar }) => {
  const params = {
    config: `
<View>
  <Text name="text" value="$text"></Text>
