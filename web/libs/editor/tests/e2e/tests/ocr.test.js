...
Scenario("Basic scenario", async ({ I, Cynaps, AtImageView, AtSettings, AtLabels, AtOutliner }) => {
  I.amOnPage("/");
  ...
  I.pressKey("Enter");
  const results = await Cynaps.serialize();
...
Scenario(
  ...
  async ({ I, Cynaps, AtImageView, AtSettings, AtLabels, AtOutliner }) => {
    I.amOnPage("/");
    Cynaps.init({ config: createConfig(), data });
    Cynaps.waitForObjectsReady();
    AtSettings.open();
    AtSettings.setGeneralSettings({
      [AtSettings.GENERAL_SETTINGS.SHOW_LABELS]: true,
    ...
    for (const region of Object.values(regions)) {
      ...
      if (region.text) {
        I.fillField(AtOutliner.locateSelectedItem().find(".CF-textarea-tag__input"), region.text);
      }
    }
    const results = await Cynaps.serialize();
    ...
    session("Deserialization", () => {
      I.amOnPage("/");
      Cynaps.init({ config: createConfig(), data, annotations: [{ id: "test", result: results }] });
      Cynaps.waitForObjectsReady();
      AtOutliner.seeRegions(regions.length);
      for (const [idx, region] of Object.entries(regions)) {
        if (region.text) {
          I.seeInField(AtOutliner.locateRegionIndex(+idx + 1).find(".CF-textarea-tag__input"), region.text);
        }
      }
    });
  },
