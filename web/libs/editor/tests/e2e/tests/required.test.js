...
Scenario("Check required param", async ({ I, Cynaps, Modals }) => {
...
Scenario("Check required param in complex config", async ({ I, Cynaps, AtOutliner, Modals }) => {
...
Scenario("Check required param with visibleWhen='choice-unselected'", async ({ I, Cynaps, Modals }) => {
  ...
  Cynaps.init(params);
  // Cynaps is reloaded, there are no new annotation from prev steps
  I.dontSee("New annotation");
  I.click('[aria-label="Annotations List Toggle"]');
  I.click('[aria-label="Create Annotation"]');
