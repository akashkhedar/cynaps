...
Scenario("Rich text initialization in hightload conditions", async ({ I, Cynaps, AtOutliner }) => {
  ...
  I.executeScript(async () => {
    new Promise((resolve) => {
      const watchCynapsReady = () => {
        const isReady = window.document.querySelector(".CF-richtext");

        if (isReady) {
          window._startTime = Date.now();
          resolve(true);
        } else {
          setTimeout(watchCynapsReady, 100);
        }
      };
