...
Scenario("Lines overlap", async ({ I, Cynaps, AtTaxonomy }) => {
...
Scenario("Add custom items", async ({ I, Cynaps, AtTaxonomy }) => {
  const params = {
    config: `
<View>
  <Text name="text" value="$text"/>
  ...
  const userLabels = await Cynaps.grabUserLabels();
  ...
  await session("Deserialization", async () => {
    ...
    Cynaps.initUserLabels(userLabels);
    ...
    const result2 = await Cynaps.serialize();
  ...
  const userLabels2 = await Cynaps.grabUserLabels();
...
Scenario("Non unique values filtering", async ({ I, Cynaps, AtTaxonomy }) => {
  I.amOnPage("/");
  Cynaps.init({
    config: `
<View>
 ...
 */
Scenario.skip("Taxonomy read only in history", async ({ I, Cynaps, AtTaxonomy }) => {
...
Scenario("Taxonomy readonly result", async ({ I, Cynaps, AtTaxonomy }) => {
  I.amOnPage("/");
  Cynaps.init({
    config: `
<View>
...
Scenario("Taxonomy per region", async ({ I, Cynaps, AtTaxonomy, AtOutliner }) => {
  const config = `
<View>
  <Text name="text" value="$text"/>
...
Scenario("Aliases in Taxonomy", async ({ I, Cynaps, AtTaxonomy }) => {
...
Scenario("Taxonomy dynamic items", async ({ I, Cynaps, AtTaxonomy }) => {
  const data = {
    text: "A text",
    options: [
      {
...
Scenario("Taxonomy maxUsages", async ({ I, Cynaps, AtTaxonomy }) => {
  const config = `
<View>
  <Text name="text" value="$text"/>
...
Scenario("Taxonomy visibleWhen", async ({ I, Cynaps, AtTaxonomy }) => {
...
Scenario("Taxonomy visibleWhen with aliases", async ({ I, Cynaps, AtTaxonomy }) => {
  const createConfig = ({ showFullPath = false, visibleWhen = "choice-selected", whenChoiceValue = "Four" } = {}) => `
<View>
  <Text name="text" value="$text"/>
  <Taxonomy required="true" name="taxonomy" toName="text" leafsOnly="true" placeholder="Select something..." showFullPath="${showFullPath}">
