...
examples.forEach((example) => {
  ...
  Scenario(`Different from_name -> ${title}`, async ({ I, Cynaps, AtOutliner, AtAudioView }) => {
    ...
    if (regionsCount) {
      const restored = await Cynaps.serialize();
  ...
  Scenario(`Nonexistent from_name -> ${title}`, async ({ I, Cynaps, AtTopbar, AtOutliner }) => {
    const params = { annotations: [{ id: "test", result }], data };
    const configTree = Utils.parseXml(config);
...
[SINGLE_TYPE, MULTIPLE_TYPE].forEach((type) => {
  Scenario(`Making labels empty -> choice="${type}"`, async ({ I, Cynaps, AtOutliner, AtAudioView, AtLabels }) => {
...
Scenario("Consistency of empty labels", async ({ I, Cynaps, AtOutliner, AtImageView, AtLabels, AtPanels }) => {
