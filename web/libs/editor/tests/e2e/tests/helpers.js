...
async function initCynaps({
  ...
  window.Cynaps.destroyAll();
  window.Cynaps = new window.Cynaps("cynaps", { interfaces, config, task, settings, ...params });
}
...
const createCynapsInitFunction = (params) => {
...
function ${createMethodInjectionIntoScript("initCynaps", initCynaps)}
const fns = {${fns.join(",")}};
const params = ${JSON.stringify(preparedParams)};
initCynaps(linkFunctions(params));
`,
  );
};
 ...
 * Sets given feature flags before CF init
...
const createAddEventListenerScript = (eventName, callback) => {
  ...
  return new Function(
    ...
    window.Cynaps.on("${eventName}",${eventName});
`,
  );
};
...
const getImageSize = () => {
  const image = window.document.querySelector('img[alt="CY"]');
...
const getImageFrameSize = () => {
  const image = window.document.querySelector('img[alt="CY"]').parentElement;
  const clientRect = image.getBoundingClientRect();
