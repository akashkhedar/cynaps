...
Scenario(
  ...
  async ({ I, Cynaps, AtOutliner, AtParagraphs, AtLabels }) => {
    ...
    const twoActionsResult = await Cynaps.serialize();
    ...
    const oneActionResult = await Cynaps.serialize();
...
Scenario("Check different cases ", async ({ I, Cynaps, AtOutliner, AtParagraphs, AtLabels }) => {
  const dialogue = [
    1, // 1
    3, // 2
    1, // 3
...
Scenario(
  ...
  async ({ I, Cynaps, AtOutliner, AtParagraphs, AtLabels }) => {
    ...
    if (filterButtonVisible === 0) {
      I.say("Filter button not found, reinitializing Cynaps");
      Cynaps.init(params);
      I.wait(1);
    }
