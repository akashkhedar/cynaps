...
async function testRegion(testType, toolAccelerator, I, Cynaps, AtImageView, AtOutliner) {
  const params = {
    config,
    data,
...
Scenario("Selected brush region should stay between tools", async ({ I, Cynaps, AtImageView, AtOutliner }) => {
  await testRegion("brush", "B", I, Cynaps, AtImageView, AtOutliner);
});

Scenario(
  "Selected Magic Wand region should stay between tools",
  async ({ I, Cynaps, AtImageView, AtOutliner }) => {
    await testRegion("magicwand", "W", I, Cynaps, AtImageView, AtOutliner);
  },
);
