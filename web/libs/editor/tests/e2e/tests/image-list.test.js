...
Scenario("Image list rendering", async ({ I, Cynaps, AtImageView }) => {
  const params = {
    config: rectConfig,
    data,
...
Scenario("Image list with page navigation", async ({ I, AtImageView, Cynaps }) => {
  const params = {
    config: rectConfig,
    data,
    annotations: [{ id: 1, result: [] }],
  ...
  const prevPageButton = locate(".CF-pagination__btn.CF-pagination__btn_arrow-left");
  const nextPageButton = locate(".CF-pagination__btn.CF-pagination__btn_arrow-right");
...
Scenario("Image list with hotkey navigation", async ({ I, AtImageView, Cynaps }) => {
  const params = {
    config: rectConfig,
    data,
    annotations: [{ id: 1, result: [] }],
...
Scenario("View All disables MIG pagination", async ({ I, AtImageView, Cynaps }) => {
  const params = {
    config: rectConfig,
    data,
    annotations: [
  ...
  const prevSelector = ".CF-pagination__btn_arrow-left";
  const nextSelector = ".CF-pagination__btn_arrow-right";
...
Scenario(
  ...
  async ({ I, AtImageView, Cynaps }) => {
    ...
    await Cynaps.resultsNotChanged(result);
  },
);

Scenario("Image list exports correct data", async ({ I, Cynaps, AtImageView }) => {
  const params = {
    config: rectConfig,
    data,
    annotations: [{ id: 1, result }],
...
Scenario("Regions are not changes when duplicating an annotation", async ({ I, Cynaps, AtImageView }) => {
  const params = {
    config: rectConfig,
    data,
    annotations: [{ id: 1, result }],
...
Scenario("No errors during brush export in MIG", async ({ I, Cynaps, AtImageView, AtLabels, AtPanels }) => {
  const params = {
    config: brushConfig,
    data,
    annotations: [{ id: 1, result: [] }],
