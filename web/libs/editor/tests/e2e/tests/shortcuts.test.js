...
Data(configParams).Scenario(
  ...
  async ({ I, Cynaps, AtOutliner, current }) => {
...
Data(configParams).Scenario("Should work with emoji.", async ({ I, Cynaps, AtOutliner, current }) => {
  const { inline } = current;
  const config = createConfig({
    rows: inline ? "1" : "3",
  });
...
Data(configParams).Scenario("Should work with existent regions.", async ({ I, Cynaps, AtOutliner, current }) => {
  const { inline } = current;
  const config = createConfig({
    rows: inline ? "1" : "3",
  });
  ...
  const initialResult = await Cynaps.serialize();
...
`;
  ...
  Data(paramsTable).Scenario("Initial focus", async ({ I, Cynaps, AtOutliner, current }) => {
