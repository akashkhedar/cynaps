import { fixSFParams } from "@cynaps/frontend-test/helpers/utils/fixSFParams";
...
class LSParamsBuilder {
  ...
  private ls: typeof Cynaps = null;

  constructor(ls: typeof Cynaps) {
    this.ls = ls;
  }
...
export const Cynaps = {
  ...
  init(params: LSParams, beforeLoadCallback?: (win: Cypress.AUTWindow) => void) {
    ...
    cy.wrap(null).then(() => {
      ...
      cy.visit("/").then((win) => {
        ...
        const Cynaps = new win.Cynaps("Cynaps", fixCFParams(win.CF_CONFIG, win));

        if (win.CF_CONFIG.eventListeners) {
          for (const [event, listener] of Object.entries(win.CF_CONFIG.eventListeners)) {
            Cynaps.on(event, listener);
          }
        }
        expect(win.Cynaps.instances.size).to.be.equal(1);
        cy.get(".cf-editor").should("be.visible");
        cy.log("Cynaps initialized");
      });
    });
  },
  ...
  /**
   * Exports current result from Cynaps's selected annotationStore
   */
  serialize() {
   ...
   * It uses inner logic of Cynaps's object tag models
   */
  waitForObjectsReady() {
    cy.window().then((win) => {
