...
export const Sidebar = {
  get outliner() {
    return cy.get(".cf-outliner");
  },
  get toolBar() {
    return this.outliner.get(".cf-view-controls");
  },
  ...
  get orderRegionsButton() {
    return this.toolBar.get(".cf-view-controls__sort button");
  },
  toggleOrderByTime() {
    this.orderRegionsButton.click();
    cy.get(".cf-dropdown").contains("Order by Time").parent().click();
  ...
  get regions() {
    return this.outliner
      .should("be.visible")
      .get(".cf-tree__node:not(.cf-tree__node_type_footer) .cf-tree-node-content-wrapper");
  },
  ...
  findByRegionIndex(idx: number) {
    return this.regions
      .find(".cf-outliner-item__index")
      .filter(`:contains("${idx}")`)
      .parents(".cf-tree-node-content-wrapper");
  },
  get hiddenRegions() {
    return this.outliner.should("be.visible").get(".cf-tree__node_hidden .cf-tree-node-content-wrapper");
  },
  ...
  hasSelectedRegions(value: number) {
    this.regions.filter(".cf-tree-node-selected").should("have.length", value);
  },
  hasSelectedRegion(idx: number) {
    this.findRegionByIndex(idx).should("have.class", "CF-tree-node-selected");
  },
  ...
  toggleRegionVisibility(idx) {
    this.regions
      ...
      .trigger("mouseover")
      .find(".cf-outliner-item__controls")
      .find(".cf-outliner-item__control_type_visibility button")
  ...
  collapseDetailsRightPanel() {
    cy.get(".cf-sidepanels__wrapper_align_right .cf-panel__toggle").should("be.visible").click();
  },
  expandDetailsRightPanel() {
    cy.get(".cf-sidepanels__wrapper_align_right .cf-panel__header").should("be.visible").click();
  },
  assertRegionHidden(idx: number, id: string, shouldBeHidden: boolean) {
    const expectation = shouldBeHidden ? "have.class" : "not.have.class";
    this.findRegionByIndex(idx).should("contain.text", id).parent().should(expectation, "CF-tree__node_hidden");
  },
};
