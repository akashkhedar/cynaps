...
const optimizer = () => {
  ...
  if (mode === "production") {
    result.minimizer.push(
      new TerserPlugin({
        parallel: false, // Disabled to avoid worker OOM on Windows
      }),
...
module.exports = composePlugins(
  ...
  (config) => {
    ...
    if (!process.env.MODE?.startsWith("standalone")) {
      config.entry = {
        main: {
          import: path.resolve(__dirname, "apps/cynaps/src/main.tsx"),
        },
      };
    ...
    config.resolve.alias = {
      ...
      "@cynaps/ui": path.resolve(__dirname, "libs/ui"),
      "@cynaps/core": path.resolve(__dirname, "libs/core"),
      "@cynaps/audio-file-decoder": path.resolve(__dirname, "libs/audio-file-decoder"),
    };
    ...
    return merge(config, {
      ...
      devServer: process.env.MODE?.startsWith("standalone")
        ...
        : {
            ...
            static: {
              directory: path.resolve(__dirname, "../cynaps/core/static/"),
              publicPath: "/static/",
            },
            devMiddleware: {
              // Always use a relative path so webpack-dev-middleware intercepts
              // /react-app/* requests before they reach the proxy
            ...
            proxy: [
              ...
              {
                // Catch-all proxy to Django — EXCLUDE /react-app/ so webpack
                // dev middleware can serve its own bundles without 404
                context: (path) => !path.startsWith("/react-app/"),
                target: `${DJANGO_HOSTNAME}`,
                changeOrigin: true,
                secure: false,
              },
