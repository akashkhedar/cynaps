...
export const Preview = ({ config, data, error, loading, project }) => {
  ...
  const currentConfig = useMemo(() => {
    // empty string causes error in CF
    return config ?? EMPTY_CONFIG;
  }, [config]);

  const initCynaps = useCallback(async (config, task) => {
    ...
    if (CF.current || !task.data) return;

    try {
      CF.current = new window.Cynaps(rootRef.current, {
        config,
        task,
      ...
      CF.current.on("presignUrlForProject", onPresignUrlForProject);
    } catch (err) {
      console.error(err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    document.getElementById("cynaps").style.opacity = opacity;
  }, [loading, error]);

  useEffect(() => {
    initCynaps(currentConfig, currentTask).then(() => {
      if (storeReady && CF.current?.store) {
        const store = CF.current.store;
