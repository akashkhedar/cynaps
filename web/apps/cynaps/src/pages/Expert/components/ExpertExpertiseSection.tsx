...
// Get badge image URL based on slugs
const getBadgeUrl = (
  categorySlug: string | null | undefined,
  specializationSlug: string | null | undefined
): string | undefined => {
  // Try specialization first, then category
  if (specializationSlug) {
    const specBadge = getBadge(specializationSlug);
    if (specBadge) return specBadge;
  }
  if (categorySlug) {
    return getBadge(categorySlug) || categoryBadges[categorySlug];
  }
  return undefined;
};
          ...
          <div className="expertise-grid">
            <AnimatePresence>
              {activeExpertise.map((exp, index) => {
                const badgeUrl = getBadgeUrl(exp.category_slug, exp.specialization_slug);
                return (
                  <motion.div
                    key={exp.id}
                  ...
                  >
                    <div className="card-icon badge-icon-container">
                      {badgeUrl ? (
                        <img src={badgeUrl} className="expertise-badge-svg" alt="badge" />
                      ) : (
            ...
            {revokedExpertise.map((exp) => {
              ...
              return (
                <div key={exp.id} className="expertise-card revoked">
                  <div className="card-icon muted badge-icon-container">
                    {badgeUrl ? (
                      <img src={badgeUrl} className="expertise-badge-svg muted" alt="badge" />
                    ) : (
                      getIcon(exp.specialization_icon || exp.category_icon)
                    )}
                  </div>
