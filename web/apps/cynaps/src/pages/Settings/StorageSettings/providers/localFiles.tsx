...
import { IconFolderOpen } from "@cynaps/icons";
import {
  Alert,
  AlertDescription,
  AlertTitle,
} from "@cynaps/shad/components/ui/alert";
    ...
    <>
      <Alert variant="destructive">
        ...
        <AlertDescription>
          ...
          and restart Cynaps to enable Local Files storage. See the
          documentation for details:{" "}
          <a
            href="https://cynaps.io/guide/storage.html#Local-storage"
            target="_blank"
            rel="noreferrer"
          >
          ...
          {isCommunityEdition && (
            <Alert variant="info">
              <AlertDescription>
                <p>
                  Tip: Create a "mydata" or "cynaps-data" directory next to the
                  command you use to run Cynaps Studio and local file serving
                  will be enabled automatically.
                </p>
                <p>
                  If you run the Docker image, the app starts in "/cynaps", so
                  you can bind-mount your host folder to "/cynaps/mydata" or
                  "/cynaps/cynaps-data" inside the container to enable local
...
export const localFilesProvider: ProviderConfig = {
  ...
  title: "Local Files",
  description:
    "Configure your local file storage connection with all required Cynaps settings",
  icon: () => (
    <IconFolderOpen
      width={40}
      height={40}
  ...
  fields: [
    ...
    },
    {
      ...
      description: `This path must be an absolute path on the host machine where Cynaps is running and start with \n"${localFilesDocumentRoot}" (LOCAL_FILES_DOCUMENT_ROOT).`,
    },
  ],
