...
export const AnnotationTestTask: React.FC<AnnotationTestTaskProps> = ({
  ...
  useEffect(() => {
    ...
    const loadCynaps = async () => {
      try {
        setIsLoading(true);
        setError(null);

        // Dynamic import of Cynaps editor
        const editorModule = await import("@cynaps/editor");
        Cynaps = editorModule.Cynaps;

        if (!Cynaps || !rootRef.current) {
        ...
        // Create Cynaps instance
        // Use taskNumber as numeric ID since Cynaps requires numeric task IDs
        sfInstance.current = new Cynaps(rootRef.current, {
        ...
        console.error("Failed to load Cynaps editor:", e);
        setError(
    ...
    const pollInterval = setInterval(() => {
      ...
      try {
        ...
        if (annotationObj) {
          ...
          // The Cynaps editor stores choices in annotationObj.results
    ...
    if (
      ...
      try {
        ...
        if (annotationObj) {
          ...
          // Cynaps editor stores selected choices in the annotation's results or areas
