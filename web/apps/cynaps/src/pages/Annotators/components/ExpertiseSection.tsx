...
import { useToast, ToastType, Spinner } from "@cynaps/ui";
import {
  Icon,
  TrophyIcon,
  PlusIcon,
...
import { getBadge, categoryBadges } from "../../../assets/badges-png";
      ...
      {badges.length > 0 && (
        <div className="badges-section">
          ...
          <div className="badges-grid">
            {badges.map((badge) => {
              // Try to get specialization badge first, then category badge
              const specializationSlug = badge.specialization?.toLowerCase().replace(/\s+/g, '-');
              const categorySlug = badge.category_slug || badge.category?.toLowerCase().replace(/\s+/g, '-');
              const badgeUrl = getBadge(specializationSlug || '') || getBadge(categorySlug || '') || categoryBadges[categorySlug || ''];
              
              return (
                <motion.div
                  key={badge.id}
                  className="badge-card badge-card-hexagon"
                  whileHover={{ y: -4, scale: 1.05 }}
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.2 }}
                >
                  <div className="badge-hexagon-container">
                    {badgeUrl ? (
                      <img src={badgeUrl} className="badge-hexagon-svg" alt="badge" />
                    ) : (
                      <div className="badge-icon-wrapper badge-fallback">
                        <Icon name={badge.icon} size={32} className="badge-icon" />
                      </div>
                    )}
                    <div className="badge-overlay">
                      <span className="badge-overlay-name">{badge.name}</span>
                      <span className="badge-overlay-category">{badge.category}</span>
                    </div>
                  </div>
                </motion.div>
              );
