...
import { useToast, ToastType } from "@cynaps/ui";
import {
  SpecialtySelection,
  AnnotationTestTask,
  TestResultsDisplay,
} from "./components";
import type { SelectedExpertise } from "./components/SpecialtySelection";
import {
...
export const AnnotatorSkillTest = () => {
  ...
  const handleSpecialtyConfirm = useCallback((specialties: string[], expertiseData: SelectedExpertise[]) => {
    setSelectedSpecialties(specialties);
    setSelectedExpertise(expertiseData);
  ...
  const handleFinishTest = useCallback(
      ...
      try {
        const response = await fetch("/api/annotators/test/submit", {
          ...
          body: JSON.stringify({
            ...
            specialties: selectedSpecialties,
            expertise: selectedExpertise.map(e => ({
              category_id: e.categoryId,
              category_slug: e.categorySlug,
              specialization_id: e.specializationId || null,
              specialization_slug: e.specializationSlug || null,
            })),
            results: {
              ...result,
        ...
        if (response.ok) {
          const data = await response.json();
          // Save earned badges from the response
          if (data.earned_badges && Array.isArray(data.earned_badges)) {
    ...
    [testCases, selectedSpecialties, selectedExpertise, attemptId]
  );
