...
// Clear localStorage of any Cynaps:settings as it may cause issues with fullscreen mode
// if coming from the old playground
localStorage.removeItem("Cynaps:settings");
...
export const PreviewPanel: FC<PreviewPanelProps> = memo(
  ({ onAnnotationUpdate }) => {
    ...
    useEffect(() => {
      ...
      function cleanup() {
        if (typeof window !== "undefined" && (window as any).Cynaps) {
          delete (window as any).Cynaps;
        }
      ...
      async function loadSF() {
        dependencies = await import("@cynaps/editor");
        Cynaps = dependencies.Cynaps;
        if (!Cynaps) return;
        cleanup();
        setShowPreview(true);
