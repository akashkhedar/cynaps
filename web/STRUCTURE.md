# Cynaps Frontend Structure Guide

> Last Updated: February 18, 2026

This document explains the organization and purpose of files in the `/web` directory, which contains the entire frontend codebase for Cynaps.

## 📁 Directory Overview

Cynaps uses **Nx** (a monorepo tool) to manage multiple applications and shared libraries. The frontend is organized into:

- **Apps**: Standalone applications (main Cynaps UI, E2E tests, Playground)
- **Libs**: Reusable shared libraries (editor, datamanager, UI components, core utilities)
...
**The main Cynaps application** - This is what users interact with.
...
End-to-end tests for the main Cynaps app (using Playwright/Cypress).
...
**Cynaps Frontend (CF)** - The annotation interface library.
...
- Standalone library that can be used outside Cynaps
...
# The built files go to: dist/apps/Cynaps/
```
...
1. **Built Assets**: When you run `yarn ls:build`, static files are created in `dist/apps/Cynaps/`
2. **Django Integration**: Django's `collectstatic` copies these to `cynaps/core/static_build/react-app/`
...
- **Cynaps Docs**: https://cynaps.xyz/docs
...
5. **Make changes**: Edit files in `apps/Cynaps/src/` or `libs/`
