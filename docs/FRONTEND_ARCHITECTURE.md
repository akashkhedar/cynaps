# Cynaps Frontend Architecture

> Last Updated: February 18, 2026

## Overview

The Cynaps frontend is an **NX monorepo** containing multiple React applications and shared libraries. It uses React 18, MobX State Tree (MST) for state management, and Tailwind CSS with Shadcn/UI for styling. The frontend is built with Webpack 5 and integrates with the Django backend via REST APIs.

### Key Features

- **Dark Theme by Default**: Full dark mode support with 5900+ design tokens
- **Multi-Portal Architecture**: Separate portals for Clients, Annotators, and Experts
- **Modular Editor**: XML-configurable annotation interface based on Label Studio
- **Data Manager**: Advanced data exploration with filtering, sorting, and bulk operations
- **Real-time Updates**: React Query for server state management with automatic cache invalidation
...
│   ├── cynaps/                 # Main application
│   ├── cynaps-e2e/             # E2E tests
│   └── playground/              # Component playground
│
├── libs/                        # Shared libraries
│   ├── editor/                  # Annotation editor (Cynaps Frontend)
...
### 1. Main Application (`apps/cynaps/`)

The primary Cynaps application that users interact with.
...
The core annotation interface - **Cynaps Frontend (SF)**.

```
libs/editor/src/
├── index.js                     # Main export
├── Cynaps.tsx                  # Main component
...
import { Typography, Card, Button } from '@cynaps/ui';
...
// Usage
import { isFeatureEnabled } from '@cynaps/core';
...
```html
<!-- cynaps/core/templates/base.html -->
<!DOCTYPE html>
<html>
<head>
  <title>Cynaps</title>
...
| Task | Location |
|------|----------|
| Add new page | `apps/cynaps/src/pages/` |
| Update navigation | `apps/cynaps/src/components/Header/` |
