...
## Overview
This document describes the storage quota enforcement and project cost (formerly security deposit) system implemented in Cynaps. The system ensures fair usage of platform resources while maintaining financial accountability for annotation projects.

### Key Components
- **Storage Quota Enforcement**: Plan-based free storage with overage charges
- **Project Cost Calculation**: Dynamic security fees based on project size
- **Credit Management**: Real-time credit balance tracking and validation
- **Billing Integration**: Razorpay payment processing for credits and subscriptions
...
3. **Overage Charges**
   - Storage exceeding free tier is charged at:
     - **Starter Plan**: 25 Credits/GB
     - **Growth Plan**: 18 Credits/GB
     - **Scale Plan**: 13 Credits/GB
...
4. **Error Handling**
   ...
   Overage: 2.50 GB × 25 Credits/GB = 62.50 Credits.
...
**File**: `cynaps/data_import/uploader.py`
...
**File**: `cynaps/billing/storage_service.py`
...
|-----------|--------------|--------------|
| 0 - 100 | 500 Credits | Small |
| 101 - 500 | 700 Credits | Medium |
| 501 - 1,000 | 900 Credits | Large |
| 1,001 - 2,500 | 1,100 Credits | Very Large |
| 2,501 - 5,000 | 1,300 Credits | Enterprise |
| 5,000+ | 1,500 Credits | Massive |
...
- Storage Fee = Estimated Storage (GB) × 10 Credits/GB
...
**Calculation:**
1. Security Fee: 700 Credits (101-500 tasks tier)
2. Storage Fee: 2 GB × 10 Credits/GB = 20 Credits
3. Annotation Rate: 5 Credits (bounding box)
4. Complexity: 1.5x (6-15 labels)
5. Buffer: 1.5x
6. Annotation Fee: 250 × 5 Credits × 1.5 × 1.5 = 2,812.50 Credits

**Total Project Cost: 700 Credits + 20 Credits + 2,812.50 Credits = 3,532.50 Credits**

### Implementation Details

**File**: `cynaps/billing/models.py`
...
**File**: `cynaps/billing/services.py`
...
**File**: `cynaps/billing/cost_estimation.py`
...
```python
logger.info(
    f"Charged storage overage: 2.50 GB = 62.50 Credits"
)
```
...
4. Verify:
   - Overage charge: 3 × 25 Credits = 75 Credits
   - Credits deducted: 75 Credits
   - Transaction created: "Storage overage charge: 3.00 GB"

### Test Security Fee Tiers

1. Create project with 50 tasks → Security fee: 500 Credits
2. Create project with 200 tasks → Security fee: 700 Credits
3. Create project with 750 tasks → Security fee: 900 Credits
4. Create project with 1,500 tasks → Security fee: 1,100 Credits
5. Create project with 3,000 tasks → Security fee: 1,300 Credits
6. Create project with 6,000 tasks → Security fee: 1,500 Credits
