# Cynaps Business Model

> Last Updated: February 18, 2026

## Executive Summary

Cynaps operates a **credits-based marketplace model** that connects enterprise clients needing data annotation with a trained workforce of verified annotators. The platform generates revenue through credit sales and subscription plans (Starter/Growth/Scale), while paying annotators a 40-50% revenue share for completed work. Quality is enforced through the Honeypot v2.0 system, multi-annotator consensus, and expert review.

### Revenue Streams

- **Credit Sales**: Pay-as-you-go credit purchases for annotation tasks
- **Subscription Plans**: Monthly/annual plans with bundled credits and storage
- **Storage Overage**: Per-GB charges for storage exceeding plan limits
- **Priority Processing**: Premium pricing tiers for faster turnaround
...
|------|---------------|---------------|----------------|---------|-------|
| **Starter** | 2,999 Credits | 2,000 | 1.50 Credits/credit | 5 GB | 2 |
| **Growth** | 7,499 Credits | 6,000 | 1.25 Credits/credit | 25 GB | 5 |
| **Scale** | 14,999 Credits | 15,000 | 1.00 Credits/credit | 100 GB | 10 |
...
| Package | Credits | Price (Credits) | Rate/Credit |
|---------|---------|-------------|-------------|
| Starter | 500 | 999 Credits | 2.00 Credits |
| Standard | 2,000 | 3,499 Credits | 1.75 Credits |
| Professional | 5,000 | 7,999 Credits | 1.60 Credits |
| Enterprise | 15,000 | 21,999 Credits | 1.47 Credits |
...
Credits are the universal currency in Cynaps:

```python
# Credits are the primary currency in Cynaps
...
```python
class ProjectBillingService:
    BASE_DEPOSIT_FEE = Decimal('500')        # Minimum 500 Credits
    STORAGE_RATE_PER_GB = Decimal('10')      # 10 Credits/GB
...
Calculation:
- Base fee: 500 Credits
- Storage fee: 10 GB × 10 Credits = 100 Credits
- Annotation: 1,000 × 5 Credits × 1.2 (complexity) × 1.5 (buffer) = 9,000 Credits

Total Deposit: 9,600 Credits
```
...
class AnnotatorEarnings:
    ...
    # Client pays: 5 Credits
    # Annotator receives: 5 × 0.45 = 2.25 Credits
    # Platform keeps: 5 × 0.55 = 2.75 Credits
```
...
class PaymentService:
    # Base rates per annotation type (Credits)
    BASE_RATES = {
...
Final Payment = 5.00 Credits × 1.3 × 0.9 × 1.1 = 6.44 Credits
```
...
1. Security Deposit:
   - Base fee: 500 Credits
   - Storage (5 GB): 50 Credits
   - Annotation estimate: 10,000 × 8 Credits × 1.5 = 120,000 Credits
   - Total deposit: 120,550 Credits
...
4. Final Cost:
   - 390,000 Credits × 1.25 Credits (Scale plan) = 487,500 Credits
   - Security deposit refunded after completion

DISTRIBUTION:
- Platform revenue: ~487,500 Credits × 0.55 = 268,125 Credits
- Annotator payments: ~487,500 Credits × 0.45 = 219,375 Credits
  - Per annotator avg: 2.19 Credits/image
```
...
class StorageBilling:
    ...
    STORAGE_TIER_RATES = {
        'standard': {'min': 5, 'max': 50, 'rate': Decimal('5')},   # 5 Credits/GB
        'large': {'min': 50, 'max': 500, 'rate': Decimal('3')},    # 3 Credits/GB
        'enterprise': {'min': 500, 'rate': Decimal('2')},          # 2 Credits/GB
    }
```
...
Client pays: 10 Credits (example)
├── Platform keeps: 5.50 Credits (55%)
│   ├── Infrastructure: 1.00 Credit
│   ├── Quality systems: 1.50 Credits
│   ├── Support/operations: 1.00 Credit
│   └── Margin: 2.00 Credits
│
└── Annotator receives: 4.50 Credits (45%)
    ├── Base payment: 3.50 Credits
    ├── Quality bonus: 0.50 Credits
    └── Trust bonus: 0.50 Credits
```
...
|-------|-----------|-----------|-------|---------|
| New | 50 | 2.40 Credits | 120 Credits | 3,600 Credits |
| Junior | 80 | 3.00 Credits | 240 Credits | 7,200 Credits |
| Regular | 100 | 3.30 Credits | 330 Credits | 9,900 Credits |
| Senior | 120 | 3.90 Credits | 468 Credits | 14,040 Credits |
| Expert | 150 | 4.50 Credits | 675 Credits | 20,250 Credits |
...
class BonusPool:
    ...
    total_amount = DecimalField()  # e.g., 10,000 Credits
