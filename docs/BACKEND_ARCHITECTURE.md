# Cynaps Backend Architecture

> Last Updated: February 18, 2026

## Overview

The Cynaps backend is built on **Django 5.1.8** with **Django REST Framework 3.15.2**, using **PostgreSQL** as the primary database and **Redis** for caching and task queues. The architecture follows a modular Django app structure with clear separation of concerns, featuring a custom UUID7-based Finite State Machine (FSM) for managing project, task, and annotation lifecycles.

### Technology Highlights

- **18+ Django Applications**: Modular design with dedicated apps for each domain
- **FSM State Management**: Custom state machine with UUID7-based state transitions and history tracking
- **Honeypot System v2.0**: System-controlled quality monitoring with automatic injection and evaluation
- **Razorpay Integration**: Payment processing for credits and annotator payouts
...
```bash
# Database
DATABASE_URL=postgres://user:pass@localhost:5432/cynaps
REDIS_URL=redis://localhost:6379/0

# Security
SECRET_KEY=your-secret-key
DEBUG=false
ALLOWED_HOSTS=cynaps.io,app.cynaps.io

# Storage
AWS_ACCESS_KEY_ID=xxx
AWS_SECRET_ACCESS_KEY=xxx
AWS_STORAGE_BUCKET_NAME=cynaps-data
