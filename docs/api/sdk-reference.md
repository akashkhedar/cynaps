# Cynaps Python SDK Reference

Complete reference documentation for the Cynaps Python SDK.
...
The `Cynaps` class is your entry point to the Cynaps API. An async version `AsyncCynaps` is also available.
...
client = Cynaps(
    api_key: str = None,
    base_url: str = "https://api.cynaps.io",
    timeout: int = 30
)
```
...
|-----------|------|---------|-------------|
| `api_key` | `str` | `None` | Your API key. If not provided, reads from `cynaps_API_KEY` env var |
| `base_url` | `str` | `https://api.cynaps.io` | API base URL |
...
# Async client
from cynaps_sdk import AsyncCynaps
async_client = AsyncCynaps(api_key="sk_live_xxxx")
```
...
except cynaps.errors.TimeoutError:
...
|--------|-------------|----------|
| `json` | Cynaps JSON format | General purpose |
...
webhook = client.webhooks.create(
    url="https://api.yourapp.com/webhooks/cynaps",
...
def handle_webhook(request):
    ...
    signature = request.headers.get("X-Cynaps-Signature")
    
    try:
        event = cynaps.webhooks.verify_signature(
            payload=payload,
            signature=signature,
            secret="whsec_xxxxxxxxxxxx"
        )
    ...
    except cynaps.errors.WebhookSignatureError:
...
except cynaps.errors.AuthenticationError as e:
    # Invalid or expired API key
    print(f"Auth failed: {e}")
    
except cynaps.errors.PermissionError as e:
    # Insufficient permissions
    print(f"Permission denied: {e}")
    
except cynaps.errors.ValidationError as e:
    # Invalid request parameters
    print(f"Validation error: {e.errors}")
    
except cynaps.errors.NotFoundError as e:
    # Resource not found
    print(f"Not found: {e}")
    
except cynaps.errors.RateLimitError as e:
    # Rate limit exceeded
    print(f"Rate limited. Retry after {e.retry_after} seconds")
    
except cynaps.errors.InsufficientCreditsError as e:
    # Not enough credits
    print(f"Need {e.required} credits, have {e.available}")
    
except cynaps.errors.CynapsError as e:
    # Generic Cynaps error
    print(f"Error: {e}")
```
