...
Receive real-time notifications when events occur in your Cynaps projects.
...
```http
POST /webhooks/cynaps HTTP/1.1
Host: api.yourapp.com
Content-Type: application/json
X-Cynaps-Signature: sha256=abc123...
X-Cynaps-Event: project.completed
X-Cynaps-Delivery: evt_abc123
X-Cynaps-Timestamp: 1673612400
User-Agent: Cynaps-Webhook/1.0
```
...
```json
{
  ...
  "data": {
    ...
    "format": "coco",
    "download_url": "https://cynaps.io/exports/exp_xyz123/download",
...
Always verify webhook signatures to ensure requests are from Cynaps.
...
@app.route("/webhooks/cynaps", methods=["POST"])
def handle_webhook():
    # Get signature from header
    signature = request.headers.get("X-Cynaps-Signature", "")
    timestamp = request.headers.get("X-Cynaps-Timestamp", "")
...
def handle_webhook():
    ...
    try:
        ...
        return jsonify({"error": "Invalid signature"}), 401
    except cynaps.errors.WebhookTimestampError:
...
Cynaps retries failed webhook deliveries with exponential backoff:
...
3. Check firewall allows Cynaps IPs
