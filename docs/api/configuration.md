...
Advanced configuration options for the Cynaps SDK and API.
...
client = cynaps.Client(
    ...
    # Optional settings
    base_url="https://api.cynaps.io",     # API endpoint
...
|----------|-------------|---------|
| `cynaps_API_KEY` | API key for authentication | - |
| `cynaps_BASE_URL` | API base URL | `https://api.cynaps.io` |
| `cynaps_TIMEOUT` | Request timeout in seconds | `30` |
| `cynaps_MAX_RETRIES` | Max retry attempts | `3` |
| `cynaps_LOG_LEVEL` | Logging level | `INFO` |
| `cynaps_VERIFY_SSL` | Verify SSL certificates | `true` |
| `HTTP_PROXY` / `HTTPS_PROXY` | Proxy settings | - |

```bash
# Set in shell
export cynaps_API_KEY=sk_live_xxxx
export cynaps_TIMEOUT=60

# Or in .env file
cynaps_API_KEY=sk_live_xxxx
cynaps_LOG_LEVEL=DEBUG
```

```python
# SDK auto-reads from environment
client = cynaps.Client()  # Uses cynaps_API_KEY
```
...
```python
from cynaps import RetryConfig
...
cynaps.set_log_level("DEBUG")

# Log to file
handler = logging.FileHandler("cynaps.log")
cynaps.add_log_handler(handler)
...
async def main():
    client = cynaps.AsyncClient(api_key="sk_live_xxxx")
...
```python
from cynaps import RateLimitConfig
...
client = cynaps.Client(
    ...
    base_url="https://sandbox-api.cynaps.io"
)
```
...
```python
from cynaps.testing import MockClient
