...
This document details the complete annotator lifecycle on Cynaps - from initial signup through task completion, quality assurance, and payments. The platform implements a sophisticated workforce management system that balances quality, speed, and annotator earnings through the Honeypot System v2.0, Trust Level progression, and automated assignment algorithms.

### System Highlights

- **7-Stage Annotator Status Flow**: pending_verification → pending_test → test_submitted → under_review → approved → active/suspended
- **5-Tier Trust System**: New → Junior → Regular → Senior → Expert with earnings multipliers (0.8x to 1.5x)
- **Dynamic Task Assignment**: Real-time overlap calculation based on available annotators (1-3)
- **Honeypot v2.0**: System-controlled injection with automatic accuracy evaluation and tiered warnings
- **Split Payment Model**: 40% on submission → 40% on consensus → 20% on expert review
...
│  │ Annotator sees "Pending: X.XX Credits"                 │    │
│  └───────────────────────────┬─────────────────────────┘    │
│                              │                               │
│  Review Approved             │                               │
│       │                      │                               │
...
│  │ Annotator sees "Available: Y.YY Credits, Pending: Z.ZZ Credits" │    │
│  └───────────────────────────┬─────────────────────────┘    │
│                              │                               │
...
# Payout constraints
PAYOUT_CONFIG = {
    'min_amount': 500,        # 500 Credits minimum
...
│  │ 2,450 Credits │  │  890 Credits │  │ 28,750 Credits │    │
│  └──────────────┘  └──────────────┘  └──────────────┘    │
│                                                            │
│  📊 This Month                                             │
│  ├── Tasks Completed: 145                                  │
│  ├── Average per Task: 18.50 Credits                     │
│  ├── Accuracy Bonus: 425 Credits                         │
│  └── Trust Level Bonus: 350 Credits (1.3x multiplier)    │
│                                                            │
│  📈 Earnings Trend                                         │
│  [Chart showing daily/weekly earnings]                     │
│                                                            │
│  📝 Recent Transactions                                    │
│  ├── Task #12345 - Image Labeling     +22.50 Credits  Today     │
│  ├── Task #12344 - Text NER           +15.00 Credits  Today     │
│  ├── Accuracy Bonus                    +50.00 Credits  Yesterday │
│  └── Payout Completed                 -2,000 Credits  Jan 10    │
│                                                            │
└────────────────────────────────────────────────────────────┘
```
